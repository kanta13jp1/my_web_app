...
### Step 5: orphan branch チェック

blog-publish の dispatch は branch 経由のため、`blog-publish/*` orphan が溜まる。
詳細は `blog-publish-cleanup` skill を参照。

```bash
count=$(git ls-remote --heads origin "blog-publish/*" | wc -l)
echo "orphan blog-publish branches: $count"
[ $count -gt 5 ] && echo "⚠️ cleanup 推奨"
```
