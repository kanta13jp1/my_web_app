...
class AssetInterestMonth {
  ...
  AssetInterestMonth({required this.month, required Map<String, int> amounts,
    required this.complete, required this.evidence, this.revision})
      : amounts = Map.unmodifiable(amounts) {
    if (!RegExp(r'^20\d{2}-(0[1-9]|1[0-2])$').hasMatch(month) ||
        amounts.isEmpty || amounts.length > 100 || evidence.trim().isEmpty ||
        evidence.length > 500 || amounts.entries.any((e) =>
          e.key.trim().isEmpty || e.key.length > 100 || e.value < 0 || e.value > 100000000)) {
      throw const FormatException('年月・口座別金額・確認根拠を確認してください');
    }
  }

  int get total => amounts.values.fold(0, (a, b) => a + b);
  Map<String, dynamic> toJson() => {'month': month, 'amounts': amounts,
    'complete': complete, 'evidence': evidence};
  factory AssetInterestMonth.fromJson(Map<String, dynamic> json, {String? revision}) =>
    AssetInterestMonth(month: json['month'] as String,
      amounts: Map<String, int>.from(json['amounts'] as Map),
      complete: json['complete'] == true, evidence: json['evidence'] as String,
      revision: revision);
  ...
  int? reductionFrom(AssetInterestMonth? previous, DateTime now) {
    ...
    if (monthKey(DateTime(date.year, date.month - 1)) != previous.month) return null;
  ...
  static Map<String, int> parseAmounts(String text) {
    ...
    for (final line in text.split('\n').where((s) => s.trim().isNotEmpty)) {
      ...
      if (parts.length != 2) throw const FormatException('各行を「口座名=利息円」で入力してください');
