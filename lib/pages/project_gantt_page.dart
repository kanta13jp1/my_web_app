...
bool canAssignWbsTaskToOwner(WbsTask task, String ownerKey) {
  ...
  return task.activeOwnerKey != key;
}

bool canUnassignWbsTaskOwner(WbsTask task) {
  if (task.isEffectivelyCompleted) return false;
  return task.activeOwnerKey != _kWbsUnassignedOwnerKey;
}
...
class _WbsAssignmentBoard extends StatelessWidget {
  ...
  List<WbsTask> _tasksForOwner(String ownerKey) {
    ...
    return tasks
        .where(
          (task) => !task.isEffectivelyCompleted && task.activeOwnerKey == key,
        )
        .toList();
  }
...
class _WbsTaskAssignmentChip extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final ownerColor = task.activeOwnerColor;
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          const SizedBox(width: 8),
          Text(
            _activeWbsShortInstance(task.activeOwnerKey),
            style: TextStyle(
              color: ownerColor,
              fontSize: 10,
