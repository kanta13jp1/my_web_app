...
class _EnglishReadingPracticePageState
  ...
  void _finishRsvp() {
    ...
    _resultElapsedMs = _rsvpWpm > 0
        ? ((words / _rsvpWpm) * 60000).round()
        : 0;
    _goToQuizOrResult();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(_mode == 'rsvp' ? 'RSVP 速読トレーナー' : '速読 計測'),
      ),
      body: _loading
  ...
  Widget _buildRsvpReading(EnglishReadingLesson lesson) {
    ...
    final current = total == 0
        ? ''
        : _rsvpWords[_rsvpIndex.clamp(0, total - 1)];
      ...
      children: [
        ...
        if (!_graded)
          SizedBox(
            ...
            child: ElevatedButton.icon(
              onPressed:
                  _answers.contains(null) ? null : _gradeQuiz,
