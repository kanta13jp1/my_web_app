    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildContent(
            '自分株式会社は Notion (CSV/API) / Evernote (ENEX) / Markdown / Excel (XLSX) / Word (DOCX) / Business CSV ファイルからのインポートに対応しています。\n'
            'MoneyForward・X (Twitter)・GitHub は直接インポート未対応のため、下記の手順でファイル形式を変換してからご利用ください。\n\n'
            '【インポート画面の開き方】\n'
            ...
            '6. ダウンロードした ZIP を解凍し、中の .csv ファイルを取り出す（インポート画面は .csv 拡張子のみ選択可能なため、ZIP のままでは選択できません）\n'
            '7. 自分株式会社のインポート画面で「ファイルを選択」→取り出した .csv を選択 →「インポート開始」をタップ\n\n'
            '━━━━━━━━━━━━━━━━━━━\n'
            ...
            '━━━━━━━━━━━━━━━━━━━\n'
            '※ MoneyForward の日本語ヘッダー（日付・内容・金額 等）は「Notion (CSV)」の列形式と異なり、取り込めない場合があります。Excel 形式に変換し、取り込み前のプレビューで件数と本文を確認してください。\n'
            '1. MoneyForward ME にログイン（moneyforward.com）\n'
            '2. 画面左上のメニューアイコン（≡）→「家計簿」→「明細」タブをタップ\n'
            '3. 画面右上の「エクスポート」ボタン →「CSV ダウンロード」を選択し、期間を指定してダウンロード\n'
            '4. ダウンロードした CSV を Excel（または Google スプレッドシート）で開き、「名前を付けて保存」→ 形式「Excel ブック (.xlsx)」で保存\n'
            '5. 自分株式会社のインポート画面で「Excel (XLSX)」を選び「ファイルを選択」→保存した .xlsx を選択\n'
            '6.「インポート開始」をタップ →「内容」列の値を本文としてノートを行ごとに取り込みます。金額・日付を会計データとして復元する機能ではありません。\n\n'
            '━━━━━━━━━━━━━━━━━━━\n'
            '【X (Twitter) からのデータ移行】\n'
            '━━━━━━━━━━━━━━━━━━━\n'
            '※ X の公式アーカイブは ZIP 内の JavaScript ファイル形式のため、インポート画面では直接読み込めません（.zip 拡張子は選択できません）。\n'
            '1. X「設定とサポート」→「設定」→「アカウント」→「データのアーカイブをダウンロード」から ZIP を取得\n'
            '2. 残したいツイートの本文をテキストエディタにコピーし、.md または .txt ファイルとして保存\n'
            '3. 自分株式会社のインポート画面で「Markdown」を選び、保存したファイルを選択 →「インポート開始」\n\n'
            '━━━━━━━━━━━━━━━━━━━\n'
            '【GitHub からのデータ移行】\n'
            '━━━━━━━━━━━━━━━━━━━\n'
            '※ GitHub のアカウントデータエクスポートは .tar.gz 形式のため、インポート画面では直接読み込めません（対応拡張子外）。\n'
            '1. GitHub「Settings」→「Archives」→「Export account data」から .tar.gz を取得、または残したい README・Issue 本文をコピー\n'
            '2. 取り込みたい内容を .md ファイルとして保存（1 リポジトリ・1 Issue = 1 ファイル推奨）\n'
            '3. 自分株式会社のインポート画面で「Markdown」を選び、保存したファイルを選択 →「インポート開始」\n\n'
            '━━━━━━━━━━━━━━━━━━━\n'
