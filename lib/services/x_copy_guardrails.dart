...
/// R23: 内容アーキタイプの実測教訓(2026-07-12 同日3連投)。独自集計データの
/// レポート型投稿が 3.2K インプレッション、ニュース要約スレが 517、ニュース→
/// 製品転換型が 28 と、投稿の「情報ペイロードの型」がコピー品質よりインプレを
/// 支配した。両生成経路(universal_x_share_service / CmoPage)がこの単一定数を
/// 参照してドリフトを防ぐ(kAppFeatureFacts と同じ集約パターン)。
const String kDataReportArchetypeLesson =
    '実測の教訓(2026-07-12 同日3連投の実測インプレッション): 独自に集計した実数を密に並べる'
    '「データレポート型」(冒頭に主要な実数→基準との差分→残り→内訳→アラート→固有名詞リスト)が 3.2K、'
    'ニュース要約スレが 517、ニュース→製品転換型が 28。読者が既に追っている固有テーマの'
    '「独自集計の実数」が最強のフック。可能な日は冒頭1行目に今日の具体的な実数を1つ置き、'
    '差分→内訳→次の節目 の順に短行で並べる。数字は実在するもののみ使い、実数が無い部分は数字を減らせ(捏造は禁止)。';
