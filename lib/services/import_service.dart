...
class ImportService {
  ...
  Future<List<Map<String, dynamic>>> parseEvernoteEnex(
    ...
    try {
      ...
      final noteMatches =
          RegExp(r'<note>(.*?)</note>', dotAll: true).allMatches(enexContent);

      for (final match in noteMatches) {
        final noteXml = match.group(1) ?? '';

        final titleMatch = RegExp(r'<title>(.*?)</title>').firstMatch(noteXml);
        final contentMatch = RegExp(r'<content>(.*?)</content>', dotAll: true)
            .firstMatch(noteXml);
        final createdMatch =
            RegExp(r'<created>(.*?)</created>').firstMatch(noteXml);
        final updatedMatch =
            RegExp(r'<updated>(.*?)</updated>').firstMatch(noteXml);
  ...
  Future<List<Map<String, dynamic>>> parseMarkdown(
    ...
    try {
      ...
      // # や ## で始まる行をタイトルとして認識
      final sections =
          markdownContent.split(RegExp(r'^#{1,2}\s+', multiLine: true));
