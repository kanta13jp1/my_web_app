...
class AssetManagementAiSummaryService {
  ...
  String _buildPrompt({
    ...
    return [
      ...
      '現在データ優先ルール: 唯一の「現在の事実」は「AIに渡す詳細ペイロード」とアクションアイテムだけです。previous_ai_analyses（metrics_snapshot）は過去時点のスナップショットで、現在の事実ではありません。履歴に出てくる金額・使用可能額・未払い・期限超過を、現在のものとして断定・督促してはいけません。現在の使用可能額がプラスなら「不足」「マイナス」と言わず、現在のアクションアイテムや支払日別リスクに無い負債、または paid が true の負債を「期限超過」「未払い」と呼ばないでください。クレジットカードやショッピング枠等の請求に含まれる負債（included_in_billing_account: true、例: auなどauPayカード払いになっているもの）は、親カード（auPAYカード等）でまとめて支払われるため、個別の未払い負債として計上・督促・未払い一覧に記載しないでください。親カードが支払済み（paid: true）であれば、その含まれる負債も支払い処理済みとして扱い、「期限超過」「未払い」として扱ってはいけません。個別の未払い負債・未払い支出として言及するのは直接支払う対象（is_direct_cashflow_target: true かつ paid: false、unpaid_direct_payments に含まれるもの）のみとしてください。支払済み（paid: true）の負債（auPayカードなど）はすでに今月分の引き落とし・支払いが完了しています。支払済みの負債を「未払い」「期限超過」として扱ったり、支払うよう督促してはいけません。「auPayカードは今月分引落済みです」のように支払完了した事実としてのみ言及してください。受取済み（received: true）の給与・収入を「未受取」「期限超過」「未着金」として扱ったり、今日着金しているか確認するよう督促してはいけません。また、予定日が今日より未来の収入について、今日入金・着金を確認するよう指示してはいけません。今月支払うべき額には推定最低支払額ではなく「今月支払予定額（scheduled_payment_amount）」を用い、解約済みサブスク（金額0円または非アクティブ）を「サブスク地獄」「未払支出」と言及してはいけません。Claude（Claude AI SUBSCRIPTION）は一度解約され、現在は月額3,000円の「Claude Pro」に変更済みです。過去の明細履歴にある旧金額（36,418円など）での言及や過大なサブスク負担としての批判は行わず、月額3,000円のProプランとして扱ってください。負債の年利（annual_rate）や負債総額は、過去の記憶ではなく必ずペイロード内の確定値（annual_rate, balance）を採用してください。カード明細のテキスト取込は任意の補助機能であり、設定内訳と請求額の差分や明細未取込を事実と相違する未確認事項として利用者に督促・不安視させないでください。特にアコムショッピングは定額リボ契約であり個別内訳と請求額の差分はリボ弁済金であるため正常です。ファミペイ等の日常利用カードについても明細未取込を問題視せず請求予定額を尊重してください。本人はカードを財布から抜き借入を停止しており、新規借入推定が0円であれば「追加借入ゼロ」は確実に達成されています。返済日前の残高不変状態を新規借入の疑いとして扱ったり、達成を「判定保留」「取引証拠の確認が必要」などと疑わないでください。現在値と履歴が矛盾する場合は必ず現在値を採用してください。',
      '証拠区分ルール: 残高差分からの新規借入は推定であり、残高増加が確認されない限り新規借入・誓約違反を疑わないでください。未受取の入金予定は未入金の証拠ではありません。予定日を過ぎても自動的に受取済みとはみなしません。received: false は受取状況の記録が未確認であることとして説明してください。予定と実績の対象期間・金額を照合するよう案内し、受取済みへの変更を断定的に要求しないでください。履歴は前回の記録として日付を示し、月が異なると確認できない限り先月と呼ばないでください。完済目標の月額は返済総額か追加額かを区別し、最低返済額への上乗せを勝手に解釈しないでください。',
  ...
  List<String> _validateGroundedResponse(
    ...
    if (RegExp(r'馬鹿|バカ|自己管理の甘さ|金銭感覚の麻痺|笑えないジョーク|鋼の星|孤高の職人').hasMatch(text)) {
      ...
      for (final segment in segments) {
        ...
        for (final keyword in unpaidLanguage) {
          ...
          while (true) {
            ...
            // 打ち消しチェック（「未払いではない」等）
            final windowEnd = matchEnd + 20 < segment.length
                ? matchEnd + 20
                : segment.length;
