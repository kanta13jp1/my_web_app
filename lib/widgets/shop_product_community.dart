...
class _ShopProductCommunityState extends State<ShopProductCommunity> {
          ...
          child: Column(
            ...
            children: [
              ...
              _section(
                ...
                children: [
                  Text('現在の配布版：${widget.product.version.isEmpty ? '未登録' : 'v${widget.product.version}'}'),
                  const Text('サイト上部のバージョン番号とは別の、ダウンロード商品の版です。',
                  ...
                  if (_model.releaseError != null)
                    ...
                    for (final release in _model.releases) _releaseTile(release),
                ...
                children: [
                  if (_model.reviewError != null)
                    ...
                    ListView.separated(
                      ...
                      separatorBuilder: (_, __) => const Divider(color: DesignTokens.divider),
                      itemBuilder: (_, index) => _reviewTile(_model.page!.items[index]),
                    ),
                    Wrap(
                      spacing: 12,
                      children: [
                        if (_model.pageNumber > 1)
                          TextButton(onPressed: _model.loading || _model.paging || _model.working ? null : _model.load,
                              child: const Text('最新の口コミへ戻る')),
                        if (_model.page!.hasMore)
                          TextButton(onPressed: _model.paging || _model.working ? null : _model.nextPage,
              ...
              Align(
                ...
                child: TextButton.icon(
                  onPressed: _model.loading || _model.working || _model.paging ? null : _model.load,
  ...
  Widget _section({Key? key, required String title, required List<Widget> children}) => Container(
        key: key,
  ...
  Widget _message(String message) => Semantics(
        liveRegion: true,
        child: Padding(padding: const EdgeInsets.symmetric(vertical: 8), child: Text(message)),
      );
  ...
  Widget _releaseTile(ShopProductRelease release) {
    ...
    return ExpansionTile(
      ...
      children: [Align(alignment: Alignment.centerLeft, child: SelectableText(release.notes))],
    );
  }
  ...
  Widget _reviewSummary(ShopReviewPage page) {
    ...
    return Semantics(
      ...
      child: ExcludeSemantics(child: Wrap(spacing: 8, crossAxisAlignment: WrapCrossAlignment.center, children: [
        const Icon(Icons.star, color: DesignTokens.orange),
        Text('${page.average!.toStringAsFixed(1)} / 5', style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
  ...
  Widget _reviewTile(ShopReview review) => Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 8),
          Text('${_date(review.updatedAt)}${review.postedVersion.isEmpty ? '' : ' ・ 投稿時の配布版 v${review.postedVersion}'}',
  ...
  Widget _ownReviewControls() {
    ...
    if (!_model.signedIn) return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('口コミ・評価の登録には、ログインと商品の購入が必要です。'),
      TextButton(onPressed: () => Navigator.of(context).pushNamed('/login'), child: const Text('ログイン')),
    ]);
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const Text('1商品につき1件。投稿内容は公開されます。個人情報を含めないでください。',
      ...
      Wrap(spacing: 12, runSpacing: 8, children: [
        if (_model.canReview)
          FilledButton.icon(
            ...
            style: FilledButton.styleFrom(backgroundColor: DesignTokens.orange, foregroundColor: DesignTokens.background),
          ),
        if (own != null)
          OutlinedButton(onPressed: _model.working ? null : _deleteReview, child: const Text('自分の口コミを削除')),
      ]),
    ]);
  }
}
  ...
  Widget build(BuildContext context) => ListenableBuilder(
        ...
        builder: (context, _) => _sessionRevision != widget.model.sessionRevision
          ...
          child: AlertDialog(
            ...
            title: const Text('口コミ・評価を登録', style: TextStyle(color: DesignTokens.textPrimary)),
            content: SizedBox(width: 480, child: SingleChildScrollView(child: Column(
              mainAxisSize: MainAxisSize.min,
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('星を1〜5で選んでください', style: TextStyle(color: DesignTokens.textPrimary)),
                Wrap(children: [
                  for (var value = 1; value <= 5; value++)
                    IconButton(
                      ...
                      onPressed: widget.model.working ? null : () => setState(() => _rating = value),
                      icon: Icon(value <= _rating ? Icons.star : Icons.star_border, color: DesignTokens.orange),
                    ),
                ]),
                TextField(
                  ...
                  decoration: const InputDecoration(
                    ...
                    counterStyle: TextStyle(color: DesignTokens.textSecondary),
                  ),
                ),
                const Text('投稿内容は公開されます。氏名・メールアドレス等の個人情報は記載しないでください。',
                    style: TextStyle(color: DesignTokens.textSecondary, fontSize: 12)),
                if (widget.model.actionError != null)
                  Semantics(liveRegion: true, child: Text(widget.model.actionError!, style: const TextStyle(color: DesignTokens.textPrimary))),
              ],
            ))),
            actions: [
              TextButton(onPressed: widget.model.working ? null : () => Navigator.pop(context), child: const Text('キャンセル')),
              FilledButton(
                onPressed: widget.model.working || _rating == 0 ? null : () async {
                  final saved = await widget.model.save(_rating, _body.text);
