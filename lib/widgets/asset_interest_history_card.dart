...
class AssetInterestHistoryCard extends StatefulWidget {
  ...
  final DateTime? now;
  const AssetInterestHistoryCard({
    super.key,
    required this.repository,
    this.now,
  });
  @override
  State<AssetInterestHistoryCard> createState() =>
      _AssetInterestHistoryCardState();
}
...
class _AssetInterestHistoryCardState extends State<AssetInterestHistoryCard> {
  ...
  Future<void> _edit(String month, AssetInterestMonth? existing) async {
            ...
            content: SizedBox(
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    const Text(
                      '明細の利息・リボ手数料の支払充当額だけを入力します。元金・未払利息・年率は含めません。残高は変更しません。',
                    ),
                    TextField(
                      controller: amounts,
                      enabled: !saving,
                    ...
                    : () async {
                        ...
                        try {
                          final record = AssetInterestMonth(
                            month: month,
                            amounts: AssetInterestMonth.parseAmounts(
                              amounts.text,
                            ),
                            complete: complete,
                          ...
                          update(() {
                            saving = false;
                            error =
                                '保存できませんでした。入力形式・接続を確認してください。同じ月が別端末で更新された場合は再読込が必要です。入力は保持しています。';
                          });
                        }
                      },
    ...
    // Keep controllers alive through the dialog's reverse transition.
  ...
  Widget build(BuildContext context) {
    ...
    final keys = List.generate(
      12,
      (i) => AssetInterestMonth.monthKey(
        DateTime(_now.year, _now.month - 11 + i),
      ),
    );
    final maximum = keys
        .map((k) => byMonth[k]?.total ?? 0)
        .fold<int>(1, (a, b) => a > b ? a : b);
    final closed =
        keys.where((k) => k != keys.last && byMonth[k] != null).toList();
    final latest = closed.isEmpty ? null : byMonth[closed.last];
    final date = latest == null ? null : DateTime.parse('${latest.month}-01');
    final previous = date == null
        ? null
        : byMonth[
            AssetInterestMonth.monthKey(DateTime(date.year, date.month - 1))];
    final reduction = latest?.reductionFrom(previous, _now);
    final comparison = reduction == null
        ? '前月比較は保留（連続する確定月・同じ対象口座が必要）'
        : reduction > 0
            ? '${latest!.month}：前月より利息負担が${_yen(reduction)}減少'
            : reduction < 0
                ? '${latest!.month}：前月より利息負担が${_yen(-reduction)}増加'
                : '${latest!.month}：前月と同額';
    return Card(
      child: Padding(
          ...
          children: [
            ...
            if (_loading)
              ...
              if (reduction != null && previous!.total > 0)
                Text(
                  '前月比 ${((latest!.total - previous.total) / previous.total * 100).toStringAsFixed(1)}%',
                ),
                      ...
                      Wrap(
                        ...
                        children: [
                          ...
                          Text(
                            byMonth[key] == null
                                ? '未登録'
                                : '${_yen(byMonth[key]!.total)} / ${key == keys.last ? '月途中' : byMonth[key]!.complete ? '照合済み' : '未照合'}',
                          ),
                          TextButton(
                            onPressed: () => _edit(key, byMonth[key]),
                            child: const Text('記録・修正'),
                          ),
                        ],
                      ),
                      if (byMonth[key] != null)
                        Semantics(
                          label: '$key 支払利息 ${_yen(byMonth[key]!.total)}',
                          child: LinearProgressIndicator(
                            value: byMonth[key]!.total / maximum,
                            minHeight: 12,
                            color: byMonth[key]!.complete && key != keys.last
                                ? Colors.teal
                                : Colors.orange,
                          ),
                        ),
