...
// ---------------------------------------------------------------------------
// Competitor monitoring helpers (absorbed from check-competitor-updates)
// ---------------------------------------------------------------------------
...
const MONITORED_COMPETITORS: MonitoredCompetitor[] = [
...
async function checkCompetitor(competitor: MonitoredCompetitor): Promise<CheckResult> {
  const start = Date.now();
  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 10000);
    const res = await fetch(competitor.url, { method: "HEAD", signal: controller.signal, redirect: "follow" });
    clearTimeout(timeoutId);
    const elapsed = Date.now() - start;
    return { name: competitor.name, url: competitor.url, status: res.status, responseTimeMs: elapsed, available: res.status >= 200 && res.status < 400 };
  } catch (_err) {
    return { name: competitor.name, url: competitor.url, status: 0, responseTimeMs: Date.now() - start, available: false };
  }
}

// deno-lint-ignore no-explicit-any
async function storeMonitoringResults(supabase: any, results: CheckResult[], checkedAt: string): Promise<void> {
  ...
  const { error } = await supabase.from("competitor_monitoring").insert(rows);
  if (error) console.error("Failed to store monitoring results:", error.message);
}
...
serve(async (req: Request) => {
  ...
  // ---- POST action:"check_updates" (absorbed from check-competitor-updates) ----
  if (req.method === "POST") {
    ...
    const targets = filterKeys
      ? MONITORED_COMPETITORS.filter((c) => filterKeys!.includes(c.key))
      : MONITORED_COMPETITORS;

    if (targets.length === 0) {
      return new Response(JSON.stringify({ error: "No matching competitors found." }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    await storeMonitoringResults(supabase, results, checkedAt);
    const availableCount = results.filter((r) => r.available).length;

    return new Response(
      JSON.stringify({ success: true, checkedAt, results, summary: { total: results.length, available: availableCount, unavailable: results.length - availableCount } }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
  }

  // ---- GET: return competitor feature data ----
  return new Response(
