  ...
  try {
    ...
    if (action.startsWith("horseracing.") || action.startsWith("wbs.")) {
      switch (action) {
        ...
        // ── WBS User Task Notification ────────────────────────────────────────
        case "wbs.notify_user_tasks": {
          // Returns pending/in_progress tasks assigned to 'user' instance and optionally posts to Slack
          const { data: userTasks, error: queryErr } = await admin
            .from("wbs_tasks")
            .select("id, category, title, status, progress, priority, end_date")
          ...
          if (tasks.length === 0) {
            return json({ success: true, count: 0, message: "No user tasks pending", tasks: [] });
          }
          ...
          const urgentTasks = tasks.filter((t: Record<string, unknown>) => {
            ...
            const daysLeft = Math.ceil((new Date(t.end_date as string).getTime() - Date.now()) / 86400000);
            return daysLeft <= 7;
          });

          const lines: string[] = [
            `📋 *自分株式会社 WBS — ユーザー手動タスク一覧* (${today})`,
            `合計 *${tasks.length}件* のタスクがあなたの手動対応を待っています。`,
            "",
          ];

          if (urgentTasks.length > 0) {
            lines.push(`🚨 *期限まで7日以内 (${urgentTasks.length}件)*`);
            for (const t of urgentTasks) {
              const task = t as Record<string, unknown>;
              lines.push(`  • [${task.category}] ${task.title} — 期限: ${task.end_date ?? "未設定"} (進捗 ${task.progress ?? 0}%)`);
            }
            lines.push("");
          }

          const otherTasks = tasks.filter((t: Record<string, unknown>) => !urgentTasks.includes(t));
          if (otherTasks.length > 0) {
            lines.push(`📌 *その他のタスク (${otherTasks.length}件)*`);
            for (const t of otherTasks.slice(0, 10)) {
              const task = t as Record<string, unknown>;
              lines.push(`  • [${task.category}] ${task.title} (進捗 ${task.progress ?? 0}%)`);
            }
            if (otherTasks.length > 10) lines.push(`  …他 ${otherTasks.length - 10} 件`);
          }

          lines.push("");
          lines.push(`🔗 WBS ダッシュボード: https://my-web-app-b67f4.web.app/project-gantt`);
