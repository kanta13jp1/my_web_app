    ...
    switch (action) {
      ...
      case "fitness.log": {
        const item = await addItem(admin, "fitness_log", userId, {
          activity: body.activity, duration_min: body.duration_min, calories: body.calories,
          distance_km: body.distance_km, notes: body.notes, logged_at: new Date().toISOString(),
        });
      ...
      case "recipe.add": {
        const item = await addItem(admin, "recipe", userId, {
          name: body.name, ingredients: body.ingredients ?? [], instructions: body.instructions,
          prep_min: body.prep_min, servings: body.servings ?? 2, tags: body.tags ?? [],
        });
        return json({ success: true, recipe: item });
      }
      case "meal.plan": {
        const item = await addItem(admin, "meal_plan", userId, {
          week_start: body.week_start, meals: body.meals ?? {}, notes: body.notes,
        });
        return json({ success: true, plan: item });
      }
      case "meal.list_plans": return json({ success: true, plans: await listItems(admin, "meal_plan", userId) });

      // ── Carbon Footprint ──────────────────────────────────────────────────────
      case "carbon.log": {
        const item = await addItem(admin, "carbon_log", userId, {
          category: body.category, activity: body.activity,
          kg_co2: body.kg_co2, date: body.date ?? new Date().toISOString().slice(0, 10),
        });
        return json({ success: true, log: item });
      }
      ...
      // ── Pet Care ──────────────────────────────────────────────────────────────
      case "pet.list": return json({ success: true, pets: await listItems(admin, "pet", userId) });
      case "pet.register": {
        const item = await addItem(admin, "pet", userId, {
          name: body.name, species: body.species, breed: body.breed,
          birth_date: body.birth_date, weight_kg: body.weight_kg,
        });
        return json({ success: true, pet: item });
      }
      case "pet.log_care": {
        const item = await addItem(admin, "pet_care_log", userId, {
          pet_id: body.pet_id, type: body.type ?? "feeding", notes: body.notes,
          logged_at: new Date().toISOString(),
        });
        return json({ success: true, log: item });
      }
      case "pet.vet_records": return json({ success: true, records: await listItems(admin, "vet_record", userId) });

      // ── Travel Itinerary ──────────────────────────────────────────────────────
      case "travel.list": return json({ success: true, trips: await listItems(admin, "trip", userId) });
      case "travel.create": {
        const item = await addItem(admin, "trip", userId, {
          title: body.title, destination: body.destination,
          start_date: body.start_date, end_date: body.end_date,
          budget: body.budget, status: "planning",
        });
      ...
      // ── Vehicle & Fleet ────────────────────────────────────────────────────────
      case "vehicle.list": return json({ success: true, vehicles: await listItems(admin, "vehicle", userId) });
      case "vehicle.add": {
        const item = await addItem(admin, "vehicle", userId, {
          make: body.make, model: body.model, year: body.year,
          plate: body.plate, mileage_km: body.mileage_km ?? 0, fuel_type: body.fuel_type,
        });
        return json({ success: true, vehicle: item });
      }
      case "vehicle.log_service": {
        const item = await addItem(admin, "vehicle_service", userId, {
          vehicle_id: body.vehicle_id, type: body.type, mileage_km: body.mileage_km,
          cost: body.cost, notes: body.notes, serviced_at: new Date().toISOString(),
        });
      ...
      case "parking.reserve": {
        const item = await addItem(admin, "parking_reservation", userId, {
          lot_id: body.lot_id, spot: body.spot, start_time: body.start_time,
          end_time: body.end_time, plate: body.plate, fee: body.fee,
        });
      ...
      case "checkin.add": {
        const item = await addItem(admin, "geo_checkin", userId, {
          lat: body.lat, lng: body.lng, place_name: body.place_name,
          category: body.category, note: body.note,
        });
        return json({ success: true, checkin: item });
      }
      case "checkin.list": return json({ success: true, checkins: await listItems(admin, "geo_checkin", userId) });

      // ── AR Navigation ─────────────────────────────────────────────────────────
      case "navigate.save_route": {
        const item = await addItem(admin, "ar_route", userId, {
          from: body.from, to: body.to, waypoints: body.waypoints ?? [],
          distance_m: body.distance_m, mode: body.mode ?? "walking",
        });
        return json({ success: true, route: item });
      }
      case "navigate.list_routes": return json({ success: true, routes: await listItems(admin, "ar_route", userId) });

      // ── Smart Home / IoT ──────────────────────────────────────────────────────
      case "smarthome.list_devices": return json({ success: true, devices: await listItems(admin, "iot_device", userId) });
      case "smarthome.add_device": {
        const item = await addItem(admin, "iot_device", userId, {
          name: body.name, type: body.type ?? "switch", ip: body.ip,
          room: body.room, protocol: body.protocol ?? "wifi", status: "offline",
        });
        return json({ success: true, device: item });
      }
      case "smarthome.control": {
        const item = await addItem(admin, "iot_command", userId, {
          device_id: body.device_id, command: body.command,
          params: body.params ?? {}, sent_at: new Date().toISOString(),
        });
        return json({ success: true, command: item });
      }
      case "smarthome.automation": {
        const item = await addItem(admin, "iot_automation", userId, {
          name: body.name, trigger: body.trigger, actions: body.actions ?? [], enabled: true,
        });
      ...
      // ── 2FA ───────────────────────────────────────────────────────────────────
      case "2fa.setup": {
        const secret = crypto.randomUUID().replace(/-/g, "").slice(0, 32).toUpperCase();
        const item = await addItem(admin, "2fa_setup", userId, {
          secret_hint: secret.slice(0, 4) + "****", method: body.method ?? "totp",
          enabled: false,
        });
      ...
      case "2fa.verify": {
        const item = await addItem(admin, "2fa_attempt", userId, {
          method: body.method ?? "totp", success: body.success ?? true,
          attempted_at: new Date().toISOString(),
        });
        return json({ success: true, verified: body.success ?? true, attempt: item });
      }
      case "2fa.status": {
      ...
      case "encrypt.send": {
        ...
        const item = await addItem(admin, "encrypted_msg", userId, {
          msg_id: msgId, to: body.to, ciphertext: body.ciphertext,
          algorithm: body.algorithm ?? "AES-256", sent_at: new Date().toISOString(),
        });
        return json({ success: true, msg_id: msgId, message: item });
      }
      case "encrypt.inbox": return json({ success: true, messages: await listItems(admin, "encrypted_msg", userId) });

      // ── OAuth / SSO ────────────────────────────────────────────────────────────
      case "oauth.list_providers": {
        const providers = ["google", "github", "apple", "twitter", "facebook", "discord", "slack"];
        const configured = providers.filter((p) => Deno.env.get(`${p.toUpperCase()}_CLIENT_ID`));
        return json({ success: true, providers: configured, all_providers: providers });
      }
      case "oauth.log_event": {
        const item = await addItem(admin, "oauth_event", userId, {
          provider: body.provider, event: body.event, success: body.success ?? true,
      ...
      case "backup.create": {
        ...
        const item = await addItem(admin, "backup_record", userId, {
          backup_id: backupId, type: body.type ?? "full", size_bytes: body.size_bytes,
          storage_url: body.storage_url, created_at: new Date().toISOString(),
        });
        return json({ success: true, backup_id: backupId, record: item });
      }
      case "backup.list": return json({ success: true, backups: await listItems(admin, "backup_record", userId) });
      case "backup.restore": {
        const item = await addItem(admin, "restore_record", userId, {
          backup_id: body.backup_id, status: "started", started_at: new Date().toISOString(),
        });
        return json({ success: true, restore: item });
      }

      // ── Cloud Storage Sync ────────────────────────────────────────────────────
      case "storage.list": return json({ success: true, files: await listItems(admin, "cloud_file", userId) });
      case "storage.add": {
        const item = await addItem(admin, "cloud_file", userId, {
          path: body.path, name: body.name, size_bytes: body.size_bytes,
          mime_type: body.mime_type, provider: body.provider ?? "supabase",
        });
        return json({ success: true, file: item });
      }
      case "storage.sync": {
        const item = await addItem(admin, "storage_sync", userId, {
          provider: body.provider, synced_count: body.synced_count ?? 0,
      ...
      case "presence.update": {
        const item = await addItem(admin, "presence", userId, {
          status: body.status ?? "online", room: body.room, custom: body.custom,
      ...
      case "notif_pref.get": {
        ...
        const defaultPrefs = { email: true, push: true, sms: false, discord: false, line: false };
        return json({ success: true, preferences: prefs[0]?.metadata ?? defaultPrefs });
      }
      case "notif_pref.set": {
        const item = await addItem(admin, "notification_pref", userId, {
          email: body.email ?? true, push: body.push ?? true,
          sms: body.sms ?? false, discord: body.discord ?? false, line: body.line ?? false,
          updated_at: new Date().toISOString(),
        });
      ...
      // ── Notification Digest ───────────────────────────────────────────────────
      case "digest.list": return json({ success: true, digests: await listItems(admin, "notification_digest", userId) });
      case "digest.create": {
        const item = await addItem(admin, "notification_digest", userId, {
          summary: body.summary, items: body.items ?? [], period: body.period ?? "daily",
      ...
      case "scheduled_notif.list": return json({ success: true, notifications: await listItems(admin, "scheduled_notif", userId) });
      case "scheduled_notif.create": {
        const item = await addItem(admin, "scheduled_notif", userId, {
          title: body.title, body: body.body, schedule_at: body.schedule_at,
          channel: body.channel ?? "push", status: "pending",
        });
      ...
      case "push.register": {
        const item = await addItem(admin, "push_token", userId, {
          token: body.token, platform: body.platform ?? "web", active: true,
        });
        return json({ success: true, registered: item });
      }
      case "push.send": {
        const item = await addItem(admin, "push_event", userId, {
          title: body.title, body: body.body, data: body.data, sent_at: new Date().toISOString(),
        });
      ...
      case "discord.send": {
        const webhookUrl = String(body.webhook_url ?? Deno.env.get("DISCORD_WEBHOOK_URL") ?? "");
        if (!webhookUrl) return json({ error: "No webhook URL provided" }, 400);
        const res = await fetch(webhookUrl, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ content: body.message, username: body.username ?? "自分株式会社" }),
        });
        const ok = res.status < 400;
        await addItem(admin, "discord_send", userId, { message: body.message, success: ok, sent_at: new Date().toISOString() });
      ...
      // ── LINE Notifications ────────────────────────────────────────────────────
      case "line.send": {
        const token = String(body.token ?? Deno.env.get("LINE_NOTIFY_TOKEN") ?? "");
        if (!token) return json({ error: "LINE_NOTIFY_TOKEN not configured" }, 400);
      ...
      case "status.get": {
        const checks = [
          { service: "Supabase DB", url: `${SUPABASE_URL}/rest/v1/`, expected: 200 },
          { service: "Edge Functions", url: `${SUPABASE_URL}/functions/v1/health-check`, expected: 200 },
        ];
        const results = await Promise.all(checks.map(async (c) => {
          const res = await fetch(c.url, { headers: { apikey: SUPABASE_ANON_KEY } }).catch(() => null);
          return { service: c.service, status: res?.ok ? "operational" : "degraded", http: res?.status ?? 0 };
        }));
        return json({ success: true, status: results.every((r) => r.status === "operational") ? "operational" : "degraded", services: results });
      }
      case "status.log_incident": {
        const item = await addItem(admin, "status_incident", userId, {
          service: body.service, severity: body.severity ?? "minor",
          description: body.description, started_at: new Date().toISOString(),
        });
        return json({ success: true, incident: item });
      }

      // ── Smart Inbox Triage ─────────────────────────────────────────────────────
      case "inbox.list": return json({ success: true, items: await listItems(admin, "inbox_item", userId) });
      case "inbox.add": {
        const item = await addItem(admin, "inbox_item", userId, {
          subject: body.subject, from: body.from, preview: body.preview,
          priority: body.priority ?? "normal", category: body.category ?? "inbox",
        });
        return json({ success: true, item });
      }
      case "inbox.triage": {
        const items = await listItems(admin, "inbox_item", userId);
        const urgent = items.filter((i) => (i.metadata as Record<string, unknown>)?.priority === "urgent");
        return json({ success: true, urgent: urgent.length, total: items.length, items: urgent.slice(0, 10) });
      }
      ...
      case "search.query": {
        ...
        await addItem(admin, "search_history", userId, { query: q, results_count: results.length });
        return json({ success: true, query: q, results });
      }
      case "search.history": return json({ success: true, history: await listItems(admin, "search_history", userId, 20) });

      // ── Family Sharing ────────────────────────────────────────────────────────
      case "family.list": return json({ success: true, members: await listItems(admin, "family_member", userId) });
      case "family.add_member": {
        const item = await addItem(admin, "family_member", userId, {
          name: body.name, email: body.email, role: body.role ?? "member",
          relation: body.relation, invited_at: new Date().toISOString(),
        });
      ...
      case "family.share_item": {
        const item = await addItem(admin, "family_shared", userId, {
          ...
          shared_with: body.shared_with, shared_at: new Date().toISOString(),
        });
      ...
      case "access.check": {
        ...
        const hasPermission = roles.some((r) => {
          const perms = ((r.metadata as Record<string, unknown>)?.permissions as string[]) ?? [];
