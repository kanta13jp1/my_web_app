...
serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      // ── Leave Management ─────────────────────────────────────────────────────
      case "leave.list": return json({ success: true, requests: await listItems(admin, "leave_request", userId) });
      case "leave.request": {
        const item = await addItem(admin, "leave_request", userId, {
          type: body.type ?? "paid", start_date: body.start_date, end_date: body.end_date,
          reason: body.reason, status: "pending", days: body.days,
        });
      ...
      // ── Performance Review ───────────────────────────────────────────────────
      case "review.list": return json({ success: true, reviews: await listItems(admin, "performance_review", userId) });
      case "review.create": {
        const item = await addItem(admin, "performance_review", userId, {
          reviewee_id: body.reviewee_id, period: body.period, rating: body.rating,
      ...
      case "compensation.list": return json({ success: true, records: await listItems(admin, "compensation", userId) });
      case "compensation.set": {
        const item = await addItem(admin, "compensation", userId, {
          employee_id: body.employee_id, base_salary: body.base_salary,
          bonus: body.bonus, effective_date: body.effective_date, currency: body.currency ?? "JPY",
        });
        return json({ success: true, record: item });
      }

      // ── Recruitment ──────────────────────────────────────────────────────────
      case "recruit.list_jobs": return json({ success: true, jobs: await listItems(admin, "job_posting", userId) });
      case "recruit.post_job": {
        const item = await addItem(admin, "job_posting", userId, {
          title: body.title, description: body.description, requirements: body.requirements,
          salary_range: body.salary_range, status: "open",
        });
        return json({ success: true, job: item });
      }
      case "recruit.list_applicants": return json({ success: true, applicants: await listItems(admin, "job_applicant", userId) });
      case "recruit.add_applicant": {
        const item = await addItem(admin, "job_applicant", userId, {
          job_id: body.job_id, name: body.name, email: body.email,
          resume_url: body.resume_url, status: "pending",
        });
      ...
      case "compliance.check": {
        const item = await addItem(admin, "compliance_check", userId, {
          domain: body.domain, items: body.items ?? [], result: body.result ?? "pass",
          notes: body.notes, checked_at: new Date().toISOString(),
        });
        return json({ success: true, check: item });
      }
      case "compliance.list": return json({ success: true, checks: await listItems(admin, "compliance_check", userId) });
      case "legal.list": return json({ success: true, documents: await listItems(admin, "legal_doc", userId) });
      case "legal.add": {
        const item = await addItem(admin, "legal_doc", userId, {
          title: body.title, type: body.type, content: body.content, effective_date: body.effective_date,
        });
      ...
      case "audit.log": {
        const item = await addItem(admin, "audit_trail", userId, {
          action: body.audit_action, resource: body.resource, details: body.details,
          ip: req.headers.get("x-forwarded-for") ?? "unknown",
          timestamp: new Date().toISOString(),
        });
        return json({ success: true, entry: item });
      }
      case "audit.list": return json({ success: true, entries: await listItems(admin, "audit_trail", userId, 100) });

      // ── CI/CD Pipeline ───────────────────────────────────────────────────────
      case "cicd.list_pipelines": return json({ success: true, pipelines: await listItems(admin, "cicd_pipeline", userId) });
      case "cicd.create_pipeline": {
        const item = await addItem(admin, "cicd_pipeline", userId, {
          name: body.name, repo: body.repo, stages: body.stages ?? [], trigger: body.trigger ?? "push",
        });
        return json({ success: true, pipeline: item });
      }
      case "cicd.run": {
        const item = await addItem(admin, "cicd_run", userId, {
          pipeline_id: body.pipeline_id, status: "running", started_at: new Date().toISOString(),
        });
      ...
      case "playground.save": {
        const item = await addItem(admin, "code_snippet", userId, {
          title: body.title, language: body.language ?? "dart",
          code: body.code, output: body.output, is_public: body.is_public ?? false,
        });
        return json({ success: true, snippet: item });
      }
      case "playground.list": return json({ success: true, snippets: await listItems(admin, "code_snippet", userId) });

      // ── API Docs ─────────────────────────────────────────────────────────────
      case "apidocs.generate": {
        const item = await addItem(admin, "api_doc", userId, {
          title: body.title, spec: body.spec, format: body.format ?? "openapi",
          version: body.version ?? "1.0.0",
        });
        return json({ success: true, doc: item });
      }
      case "apidocs.list": return json({ success: true, docs: await listItems(admin, "api_doc", userId) });

      // ── API Key Manager ──────────────────────────────────────────────────────
      case "apikey.list": return json({ success: true, keys: await listItems(admin, "api_key_entry", userId) });
      case "apikey.create": {
        const key = crypto.randomUUID().replace(/-/g, "");
        const item = await addItem(admin, "api_key_entry", userId, {
          name: body.name, key_hash: key.slice(0, 8) + "****",
          scopes: body.scopes ?? ["read"], expires_at: body.expires_at,
        });
      ...
      case "flags.set": {
        const item = await addItem(admin, "feature_flag", userId, {
          name: body.name, enabled: body.enabled ?? false,
          rollout_pct: body.rollout_pct ?? 100, conditions: body.conditions ?? {},
        });
      ...
      case "flags.check": {
        ...
        const enabled = (data?.metadata as Record<string, unknown>)?.enabled ?? false;
        return json({ success: true, name: body.name, enabled });
      }

      // ── Custom Dashboard Builder ─────────────────────────────────────────────
      case "dashboard.list": return json({ success: true, dashboards: await listItems(admin, "custom_dashboard", userId) });
      case "dashboard.create": {
        const item = await addItem(admin, "custom_dashboard", userId, {
          name: body.name, widgets: body.widgets ?? [], layout: body.layout ?? [],
        });
      ...
      // ── Data Pipeline ────────────────────────────────────────────────────────
      case "pipeline.list": return json({ success: true, pipelines: await listItems(admin, "data_pipeline", userId) });
      case "pipeline.create": {
        const item = await addItem(admin, "data_pipeline", userId, {
          name: body.name, source: body.source, destination: body.destination,
          transform: body.transform ?? [], schedule: body.schedule,
        });
        return json({ success: true, pipeline: item });
      }
      case "pipeline.run": {
        const item = await addItem(admin, "pipeline_run", userId, {
          pipeline_id: body.pipeline_id, status: "running", rows_processed: 0,
      ...
      case "cohort.list": return json({ success: true, cohorts: await listItems(admin, "cohort", userId) });
      case "search_analytics.list": return json({ success: true, queries: await listItems(admin, "search_query", userId) });
      case "search_analytics.record": {
        const item = await addItem(admin, "search_query", userId, {
          query: body.query, results_count: body.results_count ?? 0, clicked: body.clicked ?? false,
        });
        return json({ success: true, record: item });
      }
      ...
      case "forecast.create": {
        const item = await addItem(admin, "revenue_forecast", userId, {
          period: body.period, amount: body.amount, model: body.model ?? "linear",
          assumptions: body.assumptions ?? {},
        });
        return json({ success: true, forecast: item });
      }
      case "forecast.list": return json({ success: true, forecasts: await listItems(admin, "revenue_forecast", userId) });

      // ── Sitemap Analytics ─────────────────────────────────────────────────────
      case "sitemap.analyze": {
        const sitemapUrl = String(body.url ?? "");
        const res = await fetch(sitemapUrl).catch(() => null);
        const urls = res?.ok ? (await res.text()).match(/https?:\/\/[^\s<>]+/g) ?? [] : [];
        return json({ success: true, url_count: urls.length, urls: urls.slice(0, 100) });
      }
      ...
      case "onboarding.complete_step": {
        const item = await addItem(admin, "onboarding_step", userId, {
          step: body.step, completed_at: new Date().toISOString(),
        });
        return json({ success: true, step: item });
      }

      // ── CRM / Sales Pipeline ─────────────────────────────────────────────────
      case "crm.list_leads": return json({ success: true, leads: await listItems(admin, "crm_lead", userId) });
      case "crm.add_lead": {
        const item = await addItem(admin, "crm_lead", userId, {
          name: body.name, company: body.company, email: body.email,
          stage: body.stage ?? "prospect", value: body.value, notes: body.notes,
        });
      ...
      case "github.list_prs": {
        ...
        if (!token || !repo) return json({ error: "GITHUB_PAT and repo required" }, 400);
      ...
      case "growth.user_stats": {
        ...
        const recent = await admin.from("profiles")
          .select("created_at").gte("created_at", new Date(Date.now() - 7 * 86400000).toISOString());
        return json({ success: true, total_users: count ?? 0, new_this_week: recent.data?.length ?? 0 });
      }

      // ── Gantt / Task Dependency ──────────────────────────────────────────────
      case "gantt.list": return json({ success: true, tasks: await listItems(admin, "gantt_task", userId) });
      case "gantt.create_task": {
        const item = await addItem(admin, "gantt_task", userId, {
          ...
          progress: body.progress ?? 0, dependencies: body.dependencies ?? [],
      ...
      case "dependency.add": {
        const item = await addItem(admin, "task_dependency", userId, {
          task_id: body.task_id, depends_on: body.depends_on, type: body.type ?? "finish-to-start",
        });
        return json({ success: true, dependency: item });
      }

      // ── Spreadsheet Database ──────────────────────────────────────────────────
      case "sheet.list": return json({ success: true, sheets: await listItems(admin, "spreadsheet", userId) });
      case "sheet.create": {
        const item = await addItem(admin, "spreadsheet", userId, {
          name: body.name, columns: body.columns ?? [], rows: body.rows ?? [],
        });
        return json({ success: true, sheet: item });
      }
      case "sheet.add_row": {
        const item = await addItem(admin, "sheet_row", userId, {
          sheet_id: body.sheet_id, data: body.data,
        });
      ...
      case "wiki.create": {
        const item = await addItem(admin, "wiki_page", userId, {
          ...
          tags: body.tags ?? [], is_public: body.is_public ?? false,
        });
      ...
      case "integration.list": return json({ success: true, integrations: await listItems(admin, "integration", userId) });
      case "integration.add": {
        const item = await addItem(admin, "integration", userId, {
          name: body.name, type: body.type, config: body.config, status: "active",
        });
        return json({ success: true, integration: item });
      }
      case "integration.webhook": {
        const item = await addItem(admin, "webhook_event", userId, {
          integration_id: body.integration_id, event: body.event, payload: body.payload,
        });
        return json({ success: true, event: item });
      }

      // ── Email Template Builder ────────────────────────────────────────────────
      case "email_template.list": return json({ success: true, templates: await listItems(admin, "email_template", userId) });
      ...
      // ── Workflow Templates ─────────────────────────────────────────────────────
      case "workflow.list": return json({ success: true, workflows: await listItems(admin, "workflow_template", userId) });
      case "workflow.create": {
        const item = await addItem(admin, "workflow_template", userId, {
          name: body.name, steps: body.steps ?? [], triggers: body.triggers ?? [],
        });
      ...
      case "ai_write.generate": {
        ...
        const result = await res.json() as { candidates?: Array<{ content?: { parts?: Array<{ text?: string }> } }> };
        const text = result.candidates?.[0]?.content?.parts?.[0]?.text ?? "";
        await addItem(admin, "ai_write", userId, { prompt, tone, result: text });
      ...
      case "ai_workflow.list": return json({ success: true, workflows: await listItems(admin, "ai_workflow", userId) });
      case "ai_workflow.run": {
        const run = await addItem(admin, "ai_workflow_run", userId, {
          workflow_id: body.workflow_id, input: body.input, started_at: new Date().toISOString(),
        });
      ...
      case "election.search": {
        ...
        const res = await fetch(`https://newsapi.org/v2/everything?q=${encodeURIComponent(query)}&language=ja&sortBy=publishedAt&pageSize=10&apiKey=${Deno.env.get("NEWS_API_KEY") ?? ""}`)
          .catch(() => null);
        if (!res || !res.ok) return json({ error: "News API unavailable" }, 502);
        const data = await res.json();
        return json({ success: true, articles: data });
      }
      case "election.analyze": {
        const item = await addItem(admin, "election_analysis", userId, {
          region: body.region, candidates: body.candidates ?? [], analysis: body.analysis,
        });
      ...
      case "ef.test": {
        ...
        await addItem(admin, "ef_test", userId, { ef_name: efName, status: ok ? "ok" : "fail", tested_at: new Date().toISOString() });
        return json({ success: true, ef_name: efName, reachable: ok, http_status: res?.status ?? 0 });
      }
      case "ef.ui_check": {
        const item = await addItem(admin, "ef_ui_check", userId, {
          ef_name: body.ef_name, ui_connected: body.ui_connected ?? false, notes: body.notes,
        });
        return json({ success: true, check: item });
      }
