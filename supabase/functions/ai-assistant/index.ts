...
serve(async (req) => {
    ...
    try {
        ...
        // --- ★ 追加: 自発的介入機能 (Proactive Intervention) ---
        if (action === 'proactive_intervention') {
            const userStats = requestData.userStats || {};
            const prompt = `
            あなたは「自分株式会社」の専属コーチです。
            ユーザーの統計データに基づき、適切な「介入（メッセージ）」を生成してください。

            統計データ:
            - 現在のレベル: ${userStats.current_level}
            - 継続日数: ${userStats.current_streak}日
            - 総獲得ポイント: ${userStats.total_points}
            - 記入メモ数: ${userStats.notes_created}

            指示:
            - ユーザーが停滞していれば励まし、順調なら称賛してください。
            - 短く、心に響くメッセージにしてください（60文字以内）。
            - 次のアクションを具体的に1つ提案してください。

            以下のJSONフォーマットのみで出力してください:
            {
                "message": "メッセージ本文",
                "type": "encouragement" | "warning" | "praise",
                "suggested_action": "具体的な行動"
            }
            `;
