-- Add the anonymous mobile funnel stages and expose every primary metric
-- required by the ten landing-page hypotheses.
ALTER TABLE public.landing_experiment_events
  DROP CONSTRAINT IF EXISTS landing_experiment_events_stage_check;

ALTER TABLE public.landing_experiment_events
  ADD CONSTRAINT landing_experiment_events_stage_check
  CHECK (
    stage IN (
      'view',
      'mobile_view',
...
BEGIN
  ...
  IF p_event_key IS NULL
     ...
     OR p_event_key !~ '^lp_exp_h(0[1-9]|10)_(control|treatment)_(view|mobile_view|hero_cta|intent|trial|trial_fallback|save_cta|signup_submit|mobile_signup_submit|signup_complete|sticky_cta|feature_outcome_trial|feature_catalog_expand)$' THEN
...
CREATE OR REPLACE VIEW public.landing_experiment_arm_stats
...
SELECT
  ...
  COUNT(DISTINCT event.visitor_id)
    FILTER (WHERE event.stage = 'hero_cta') AS unique_hero_ctas,
  COUNT(DISTINCT event.visitor_id)
    FILTER (WHERE event.stage = 'intent') AS unique_intents,
  COUNT(DISTINCT event.visitor_id)
    FILTER (WHERE event.stage = 'mobile_view') AS unique_mobile_views,
  COUNT(DISTINCT event.visitor_id)
    FILTER (
      WHERE event.stage = 'mobile_signup_submit'
    ) AS unique_mobile_signup_submits,
  COUNT(DISTINCT event.visitor_id)
    FILTER (WHERE event.stage = 'sticky_cta') AS unique_sticky_ctas
