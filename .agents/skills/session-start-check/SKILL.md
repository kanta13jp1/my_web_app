...
case "$WORKTREE" in
  ...
  *)
    ...
    echo "   bash .Codex/scripts/setup-instance-worktree.sh <vscode|win|ps>"
    echo "   cd .Codex/worktrees/instance-<NAME>"
    ;;
esac
```

**期待結果**: `.Codex/worktrees/instance-<NAME>` で作業中。それ以外は即切替。
別インスタンスの `git stash` / `git pull --rebase` で uncommitted 変更が消えるリスクを根本回避。

### Step 0.5: inject-rules.txt drift 確認 (Win版#132 part 135)

```bash
# canonical (= repo) と home (= ~/.Codex/hooks/) の drift 確認
