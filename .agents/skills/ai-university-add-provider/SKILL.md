...
description: |
  ...
  - Run the periodic "Step 0" provider evaluation from the AGENTS.md ai-university-update task
  ...
  - Add (provider name as arg): research → SQL migration → yml update → AGENTS.md update → commit
---
...
`AGENTS.md` の「現在の登録プロバイダー」セクションを読む。
...
### Step 4: AGENTS.md の登録プロバイダーリストを更新

`AGENTS.md` の以下のセクションを探して更新:
...
```bash
cd [project-root]
npx markdownlint-cli --dot "AGENTS.md" ".github/COMPRESSED_PROMPT_V3.md"
```
...
```bash
git add supabase/migrations/[新ファイル] AGENTS.md .github/COMPRESSED_PROMPT_V3.md .github/workflows/ai-university-update.yml
...
- プロバイダー数は AGENTS.md から動的に読み取る（ハードコードしない）
