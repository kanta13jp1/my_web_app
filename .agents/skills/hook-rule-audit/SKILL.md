...
description: |
  月 1 回実行する `~/.Codex/hooks/inject-rules.txt` の rule audit skill。
...
`~/.Codex/hooks/inject-rules.txt` の rule (現在 20 個 / 134 行) を月 1 回監査し、
以下を実施:
...
```bash
wc -l ~/.Codex/hooks/inject-rules.txt
grep -c "^\[" ~/.Codex/hooks/inject-rules.txt
grep -oE "^\[[A-Z-]+[0-9]*\]" ~/.Codex/hooks/inject-rules.txt | sort -u
```
...
for rule in $(grep -oE "^\[[A-Z-]+[0-9]*\]" ~/.Codex/hooks/inject-rules.txt); do
  count=$(grep -rl "$rule" "/c/Users/kanta/.Codex/projects/C--Users-kanta-GitHub-my-web-app/memory/" 2>/dev/null | wc -l)
...
```bash
grep -oE "Win[#版]+[0-9]+" ~/.Codex/hooks/inject-rules.txt | sort -u
```

Win 番号が 50+ 古い = stale 可能性 → 対応する AGENTS.md / docs を読み rule が今も有効か確認。
...
```bash
ls -t /c/Users/kanta/.Codex/projects/C--Users-kanta-GitHub-my-web-app/memory/feedback_correction_*.md | head -10
```
...
ユーザーが `apply` と回答した項目を `~/.Codex/hooks/inject-rules.txt` に Edit:

1. backup: `cp ~/.Codex/hooks/inject-rules.txt ~/.Codex/hooks/inject-rules.txt.bak.YYYYMMDD`
2. Edit (削除/merge/追加)
3. 検証: `grep -c "^\[" ~/.Codex/hooks/inject-rules.txt` で rule 数確認
...
- Win版#98 (UserPromptSubmit hook 導入) / Win版#131 (AGENTS.md → hook migration)
