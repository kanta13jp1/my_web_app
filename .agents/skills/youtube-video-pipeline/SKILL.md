...
description: End-to-end reference-video workflow for fetching public captions and metadata, writing an original Japanese narration, recording the user's voice, rendering a verified 1080p video and thumbnail, uploading privately to a verified YouTube channel, publishing after a separate confirmation, registering the public video in my_web_app AI大学 as a discoverable embedded lesson, diagnosing unclickable embedded playback, and monitoring the reviewed production deployment through live playback verification. Use for requests such as 「参考動画から解説動画を作って」, 「読み上げを録音して動画化」, 「YouTubeに非公開投稿して確認後に公開」, 「公開動画をAI大学に埋め込んで」, 「再生ボタンが押せない」, 「mainへマージしてデプロイを監視」, or the complete reference-video-to-AI大学 pipeline.
---
...
Read [references/ai-university-release-operations.md](references/ai-university-release-operations.md) when embedded playback is visible but not interactive, CI fails, system memory is constrained, a merge command reports a local worktree error, or production must be verified through the exact deployed commit and real playback.
...
Follow the operational loop in [references/ai-university-release-operations.md](references/ai-university-release-operations.md). In particular, diagnose iframe hit testing before changing the player, rerun the focused route and playback tests after every fix, inspect the merged PR state even when `gh pr merge` exits nonzero, and match the deployment run by squash-merge SHA.
...
Merge only after all required checks pass. If a check fails, inspect the failed step, fix forward on the same branch, rerun focused validation, and wait for the new complete check set. Capture the actual squash merge commit, then identify the `Deploy to Production` run whose `headSha` matches it; do not assume the newest run belongs to this release.
...
Do not claim full completion until the active database lesson, production version, discoverability banner, iframe hit target, and advancing playback are all confirmed. If browser control is unavailable, report the deployment as successful but live playback QA as pending.
...
- `references/ai-university-release-operations.md`: iframe hit-test diagnosis, CI repair loop, memory-safe validation, merge edge cases, exact-SHA deployment monitoring, and truthful production playback QA.
