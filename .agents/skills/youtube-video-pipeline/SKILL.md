...
Treat Google Flow as an optional improvement candidate for a short lesson-specific clip, reference-guided motion, an approved video-to-video edit, or a multi-clip scene assembled in Scenebuilder. Do not use it automatically. When the user selects Google Flow, read [references/google-flow-video-improvement.md](references/google-flow-video-improvement.md) before signing in, uploading media, submitting a generation, consuming Google Flow or AI credits, or integrating the result. Treat Google Flow Web credits and a separately billed Veo Developer API as different routes; never infer one entitlement, balance, or approval from the other.

Google Flow Web uses a Slate rich-text prompt editor whose visible DOM can disagree with its internal submitted state. **Never use browser automation, JavaScript injection, synthetic keyboard events, programmatic paste, `fill`, or equivalent automation to enter or replace a paid Flow prompt, and never let automation click Generate.** Codex may prepare and hash the prompt, inspect settings, and review or download results, but the user must manually paste/type the approved prompt and manually click Generate. One click consumes the one-attempt approval even when the request fails, stalls, or produces the wrong content. After any prompt mismatch or unrelated output, lock the Flow Web route, inspect history and credit usage, and do not retry until a new exact cost confirmation is given; every retry remains manual-only. Use the separately billed Veo API instead only when the user explicitly selects that route and its entitlement, price, input, and budget are independently verified.

Treat Topaz Video AI as an optional finishing and restoration candidate for upscaling, artifact reduction, denoising, deblurring, stabilization, frame interpolation, or SDR-to-HDR conversion. It does not create lesson-specific scenes or improve semantic alignment. Do not use it automatically. When the user selects Topaz, read [references/topaz-video-ai-improvement.md](references/topaz-video-ai-improvement.md) before installing or authenticating, processing locally, uploading media for cloud rendering, consuming cloud credits, or integrating the result. Prefer a native high-resolution re-render for editable vector graphics and typography; preserve the verified baseline and accept Topaz output only after text, motion, audio, and full-video checks prove a material improvement.
...
- `references/google-flow-video-improvement.md`: optional Google Flow Web generation, reference-guided motion, short video editing, Scenebuilder assembly, credit confirmation, preservation, and regression checks.
- `references/topaz-video-ai-improvement.md`: optional Topaz finishing and restoration for low-quality raster footage, with native-render preference, local/cloud routing, privacy and cost gates, preservation, and regression checks.
