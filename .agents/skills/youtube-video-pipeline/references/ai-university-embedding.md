...
Report separately: YouTube state, database lesson state, deployed commit, workflow URL, banner state, iframe hit-test state, and advancing playback state. If browser control is unavailable, mark live playback QA as pending rather than claiming full completion.
