...
description: |
  毎セッション実行する GitHub Actions ワークフロー健全チェック (AGENTS.md Rule 9/17)。
...
echo "Codex/*: $(git ls-remote --heads origin 'Codex/*' | wc -l)"
```

いずれかが 5本超なら一括削除:

```bash
for pattern in "blog-publish/*" "cs-check-*" "ai-university-update/*" "daily-report-*" "youtube-analysis-*" "Codex/*"; do
...
- 毎セッション 1回 (AGENTS.md Rule 17 — 「Task: cs-check 内」または独立実行)
- 失敗 WF 発覚時 (ad hoc)
