...
await new Promise((resolve,reject)=>{worker.on('error',reject);const timer=setInterval(()=>{advanceClock();if(finished(g,lastCourse)||g.clock>=MAX_FRAMES){clearInterval(timer);resolve();return;}
 ...
 if(!busy&&g.clock>=nextDecision){nextDecision=g.clock+6;const t=performance.now(),raw=studentAction?studentAction(g):rawApi;if(mode==='raw'){effective=raw??'noop';decisions++;if(raw!==null)accepted++;else fallbackDecisions++;trace.push({frame:g.clock,x:g.p.x,raw,effective,accepted:raw!==null,age_ms:performance.now()-t});}else{busy=true;worker.postMessage({state:structuredClone(g),raw,issued:performance.now(),effective,epoch,forecastFrames:Math.max(1,Math.min(12,Math.round(lastAge*.06)))});}}},4);});
