...
function advanceClock(){const target=Math.min(3600,Math.floor((performance.now()-start)*.06));while(g.frames<target&&g.phase==='playing'){const a=mode==='assisted'?edge(g,effective):effective;if(a!==effective)releases++;g.buttons(a);g.step();g.drainSounds();}}
