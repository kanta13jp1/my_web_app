...
export async function query(lane,state,signal){
 ...
 const response=await fetch(cloud?'https://api.typesafe.ai/v1/systemone':'http://127.0.0.1:8080/v1/systemone',{method:'POST',redirect:'error',headers:{'Content-Type':'application/json',...(cloud?{Authorization:`Bearer ${process.env.JEV_MARIO_EXPERIMENT_KEY}`}:{})},body:JSON.stringify({model:cloud?'jev-latest':'localjev-latest',state,questions:{controller:{type:'choice',criteria:teacher.teacher_criteria,instructions:teacher.teacher_instructions.replace('6 simulation frames (100 ms','30 simulation frames (500 ms')}}}),signal:AbortSignal.any([signal,AbortSignal.timeout(cloud?10000:60000)])});
 ...
 return {choice:a.choice,probabilities:a.probabilities,model:body.model,uniform:new Set(values).size===1};
}
