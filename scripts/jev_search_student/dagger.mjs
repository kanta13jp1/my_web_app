...
fs.writeFileSync(`${OUT}/evaluation.json`,JSON.stringify({scope:SCOPE,feature_version:FEATURE_VERSION,training_seeds:TRAIN,held_out_seeds:HELD_OUT,
 note:'Independent recreation, not the ROM. Seed 0 = published layout (also used for training). Held-out seeds shift each enemy by up to +-4px. Course 3 was never used for training. Student = LightGBM alone, deciding every frame, labelled every 4 frames during training; teacher = exact-state search every 6 frames with per-frame jump release (privileged).',
