...
export class World11 {
  ...
  die(){if(this.phase!=='playing')return;this.sound('death');this.phase='dead';this.deaths++;this.lives--;this.input={};}
  ...
  step(){if(this.phase!=='playing')return;const p=this.p,k=this.input;this.frames++;this.time=Math.max(0,400-Math.floor(this.frames/24));if(!this.time){this.die();return;}
    ...
    if(k.jump&&!this.wasJump&&p.grounded){this.sound('jump');p.vy=Math.abs(p.vx)>1.6?-5.7:-5.2;p.grounded=false;}
    ...
    for(const e of this.enemies){if(e.dead||e.x>this.camera+280||e.x<this.camera-32)continue;
      ...
      if(e.y>250){e.dead=1;continue;}if(overlap(p,e)&&this.phase==='playing'){
        ...
        else if(p.vy>=0&&beforeY+p.h<=e.y+6){this.sound('stomp');p.y=e.y-p.h;p.vy=-3.5;this.score+=100;if(e.kind==='koopa'){e.kind='shell';e.vx=0;}else if(e.kind==='shell'){e.vx=e.vx?0:(p.x<e.x?4:-4);}else e.dead=1;}
    ...
    if(this.room==='overworld'&&p.x>=198*16&&this.phase==='playing'){this.sound('clear');this.phase='won';this.score+=Math.max(100,5000-Math.floor(p.y)*20);this.input={};}
  }
