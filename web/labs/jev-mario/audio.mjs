...
export class GameAudio{
 ...
 tick(room='overworld',{star=false,hurry=false}={}){
  ...
  const track=star?'star':room==='castle'?'castle':(room==='underground'||room==='stage-underground')?'underground':'overworld';
  ...
  const step=(track==='star'?.095:track==='underground'?.18:track==='castle'?.13:.145)*(hurry?.78:1),melody=scores[track];
