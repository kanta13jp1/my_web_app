...
| データ種別 | 保管期間 | 削除 / 匿名化方法 |
|----------|--------|----------------|
| アカウント識別情報 / 認証セッション | 退会申請から30日の取消猶予まで | 猶予終了後に Supabase Auth ユーザーと更新用セッションを削除 |
| ユーザー作成コンテンツ / 設定 / AI 入力 | アカウント存続中。退会申請後は最長30日 | Auth ユーザーに連結した行を物理削除。未分類の参照がある場合は自動処理を停止して再調査 |
...
2. 本人確認のため直近15分以内の再ログインと、画面に表示される確認文の入力が必要です。
3. 申請後30日間は同じ画面から取り消せます。取消猶予後に自動削除を開始し、開始後は取り消せません。
4. 自動処理は決済顧客情報、Storage、Supabase Auth、関連データの順序と残存検査を実施します。部分失敗時は完了扱いにせず、安全に再試行します。
