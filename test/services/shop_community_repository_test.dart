...
void main() {
  test('release request selects only public fields and limits the history',
      () async {
  ...
  test('public review RPC includes the keyset cursor without user identifiers',
      () async {
  ...
  test(
      'write uses the invoker RPC and does not send owner, version or moderation flags',
      () async {
    final repository = repositoryWith((request) async {
      expect(request.url.path, '/rest/v1/rpc/save_shop_product_review');
      expect(
        jsonDecode(request.body),
        {'p_product_id': 'test', 'p_rating': 4, 'p_body': '文章'},
      );
  ...
  test('delete scopes by product and ID and requires a returned row', () async {
    ...
    await expectLater(
      repository.deleteReview('test', 'review'),
      throwsStateError,
    );
  });
  test('logged-out own-review lookup does not call the API', () async {
    final repository =
        repositoryWith((_) async => throw StateError('unexpected request'));
