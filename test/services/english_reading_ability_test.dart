...
void main() {
  group('EnglishReadingMetrics.wpm', () {
    test('computes words per minute', () {
      expect(
        EnglishReadingMetrics.wpm(wordCount: 250, elapsedMs: 60000),
        250,
      );
      expect(
        EnglishReadingMetrics.wpm(wordCount: 250, elapsedMs: 30000),
        500,
      );
    });
  ...
  group('EnglishReadingAbilitySummary.fromAttempts', () {
    ...
    test('marks a level cleared only at target wpm and >=75% comprehension',
        () {
      final attempts = <EnglishReadingAttempt>[
