    ...
    final transport = SupabaseClient('https://example.supabase.co', 'test-key',
      accessToken: () async => 'user-a-jwt', httpClient: MockClient((request) async {
        ...
        if (request.method == 'POST') {
          if (stored != null) return http.Response('{"code":"23505","message":"duplicate"}', 409, request: request, headers: {'content-type': 'application/json'});
        ...
        if (request.method == 'PATCH') {
          ...
          stored = Map<String, dynamic>.from(jsonDecode(request.body) as Map);
          return http.Response(jsonEncode([{'pref_key': stored!['pref_key']}]), 200, request: request,
            headers: {'content-type': 'application/json'});
        }
        return http.Response(jsonEncode(stored == null ? [] : [stored]), 200, request: request,
          headers: {'content-type': 'application/json'});
      }));
