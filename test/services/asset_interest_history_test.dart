...
void main() {
  AssetInterestMonth record(String month, int amount, {bool complete = true, String name = 'A'}) =>
    AssetInterestMonth(month: month, amounts: {name: amount}, complete: complete, evidence: '明細');
  ...
  test('compares only adjacent closed complete months with same scope', () {
    ...
    expect(aug.reductionFrom(record('2026-07', 100, complete: false), now), isNull);
    expect(record('2026-08', 80, complete: false).reductionFrom(record('2026-07', 100), now), isNull);
  ...
  test('rejects invalid month, negative amount and missing evidence', () {
    ...
    expect(() => AssetInterestMonth(month: '2026-08', amounts: {}, complete: true, evidence: 'x'), throwsFormatException);
  });
