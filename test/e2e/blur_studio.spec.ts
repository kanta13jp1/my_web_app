...
test('adjust background, disable effect, and recover with reset', async ({ page }, info) => {
  ...
  await expect(page.getByRole('status')).toHaveText('Apricot · 1 px');
  await page.getByLabel('色の組み合わせ').selectOption('iris');
  await expect(page.getByRole('status')).toHaveText('Iris · 1 px');
  await page.getByLabel('ぼかし効果を使う').uncheck();
  await expect(slider).toBeDisabled();
  await expect(page.getByRole('status')).toHaveText('Iris · ぼかしなし');
  ...
  await expect(page.getByRole('status')).toHaveText('Apricot · 28 px');
...
test('narrow viewport, larger text, and reduced motion retain usable controls', async ({ page }, info) => {
  ...
  await expect(page.getByRole('status')).toContainText('ぼかしなし');
