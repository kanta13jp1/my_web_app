...
const product = ShopProduct(id: 'test', nameJa: 'テスト用アプリ', summaryJa: '説明',
  priceJpy: 500, version: '1.0', fileSizeBytes: 1024, sha256: 'abc', isPurchasable: true);

Future<void> pumpCommunity(WidgetTester tester, FakeShopCommunity repository,
...
void main() {
  testWidgets('shows application version, verified release and unknown date', (tester) async {
    final repo = FakeShopCommunity()..releaseItems = const [ShopProductRelease(
      id: 'r', version: '1.0', title: '確認済みの配布内容', notes: '更新の説明', sha256: 'abc')];
  ...
  testWidgets('unpaid visitor can read but not open the editor', (tester) async {
  ...
  testWidgets('mobile write, edit, reload and confirmed delete', (tester) async {
    final repo = FakeShopCommunity();
    await pumpCommunity(tester, repo, width: 360);
    await tapText(tester, '口コミ・評価を書く');
    expect(tester.widget<FilledButton>(find.widgetWithText(FilledButton, '公開して保存')).onPressed, isNull);
    await tester.tap(find.byKey(const ValueKey('review-star-5')));
    await tester.enterText(find.byKey(const ValueKey('review-body')), '分かりやすいゲームです。');
    ...
    await tester.enterText(find.byKey(const ValueKey('review-body')), '追記しました。');
    await tapText(tester, '公開して保存');
  ...
  testWidgets('service failure has a retry and never displays zero', (tester) async {
  ...
  testWidgets('long content remains readable at 360px', (tester) async {
    final repo = FakeShopCommunity()..publicItems.add(FakeShopCommunity.review('long', body: '詳細な口コミです。' * 100));
    await pumpCommunity(tester, repo, width: 360);
