...
NoteVersionSummary version(int index, {bool evernote = false, bool verified = false}) =>
    NoteVersionSummary(
...
class _Repository implements NoteVersionHistoryRepository {
  ...
  @override
  Future<NoteVersionPage> loadPage(String noteId, {NoteVersionCursor? before}) async {
    pageCalls++;
    if (pageGate != null) return pageGate!.future;
    if (failPage) throw StateError('private source must not be shown');
    final offset = before == null ? 0 : items.indexWhere((v) => v.id == before.id) + 1;
  ...
  Future<NoteVersionDetail> loadDetail(String noteId, String versionId) async {
    ...
    return detail ?? NoteVersionDetail(
...
Future<void> openSheet(WidgetTester tester, _Repository repository) async {
...
void main() {
  testWidgets('31st version is reachable without loading every body', (tester) async {
    ...
    await tester.scrollUntilVisible(
      find.text('さらに古い履歴を読み込む'), 500, scrollable: scrollable,
    );
    await tester.tap(find.text('さらに古い履歴を読み込む'));
    await tester.pumpAndSettle();
    await tester.scrollUntilVisible(find.text('History 30'), 300, scrollable: scrollable);
  ...
  testWidgets('failed preview retries without restoring or hiding the error', (tester) async {
    final repository = _Repository()..items = [version(1)]..failDetail = true;
    await openSheet(tester, repository);
  ...
  for (final verified in [false, true]) {
    testWidgets('Evernote preview is read-only, verified=$verified', (tester) async {
      ...
      final repository = _Repository()
        ...
        ..detail = NoteVersionDetail(
          ...
          tags: ['retained-tag'],
          attachments: [
            const NoteVersionAttachment(fileName: 'original.pdf', mimeType: 'application/pdf', fileSize: 123),
          ],
        );
      await openSheet(tester, repository);
      await tester.tap(find.text('History 1'));
      ...
      expect(find.text(verified ? 'Evernote履歴：検証済み' : 'Evernote履歴：未検証'), findsOneWidget);
  ...
  for (final width in [320.0, 1280.0]) {
    testWidgets('preview remains scrollable at width $width and large text', (tester) async {
      ...
      final repository = _Repository()
        ..items = [item]
        ..detail = NoteVersionDetail(summary: item, content: List.filled(50, 'Long historical body').join('\n'));
      await openSheet(tester, repository);
      await tester.tap(find.text('History 1'));
      await tester.pumpAndSettle();
