  ...
  await page.context().route('**/*', async (route) => {
    ...
    if (url.origin === apiOrigin) {
      ...
      if (url.pathname === '/rest/v1/rpc/get_my_shop_product_review') {
        return reply(route, { review: null, can_review: !!options.signedIn && purchased });
      }
...
test('distribution version and review navigation preserve owned download', async ({ page }, info) => {
  const fixture = await installFixture(page, { signedIn: true, purchased: true });
  await openShop(page);
  await expect(page.getByText('配布版 vfixture', { exact: true })).toBeVisible();
  const reviews = page.getByRole('button', { name: '口コミ・評価', exact: true });
  await reviews.scrollIntoViewIfNeeded();
  await reviews.click();
  await expect(page.getByText('評価はまだありません（0件）', { exact: true })).toBeVisible();
  await expect(page.getByRole('button', { name: '口コミ・評価を書く', exact: true })).toBeEnabled();
  await capture(page, info, 'owned-product-review-empty');
