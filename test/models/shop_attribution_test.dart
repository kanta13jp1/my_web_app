...
void main() {
  ...
  test('all three ASCII labels have a single canonical representation', () {
    final labels = ShopAttribution.fromUri(
      Uri(
        path: '/shop/product',
        queryParameters: {
          'utm_source': ' X ',
    ...
    expect(
      ShopAttribution.fromUri(Uri(queryParameters: labels.toQuery()))
          .toRequest(),
      labels.toRequest(),
    );
  });
  ...
  test('invalid values never alias another post or become direct', () {
    for (final value in [
      ...
      for (final key in ['utm_source', 'utm_campaign', 'utm_content']) {
        final labels =
            ShopAttribution.fromUri(Uri(queryParameters: {key: value}));
  ...
  test('duplicate query keys are ambiguous even if their values match', () {
    for (final key in ['utm_source', 'utm_campaign', 'utm_content']) {
      expect(
        ShopAttribution.fromUri(Uri.parse('/?$key=a&$key=a')).isValid,
        isFalse,
      );
    }
  });
  ...
  test('different posts and campaigns remain distinct', () {
    expect(
      ShopAttribution.parse(contentId: 'post-a').toRequest(),
      isNot(ShopAttribution.parse(contentId: 'post-b').toRequest()),
    );
    expect(
      ShopAttribution.parse(campaign: 'a').toRequest(),
  ...
  test('two login locations retain their own product and campaign', () {
    ...
    expect(
      ShopLoginContinuation.productUri(first)!.queryParameters,
      containsPair('utm_content', 'post-a'),
    );
    expect(
      ShopLoginContinuation.productUri(second)!.queryParameters,
  ...
  test('invalid labels survive login without keeping raw private values', () {
    ...
    expect(
      ShopAttribution.fromUri(ShopLoginContinuation.productUri(login)!).isValid,
      isFalse,
    );
  });

  test(
      'arbitrary redirect, purchase, identity and token parameters are ignored',
      () {
    final product = ShopLoginContinuation.productUri(
      Uri.parse(
  ...
  test('invalid product and non-login paths cannot become a return target', () {
    for (final uri in [
      '/login',
      '/login?shop_product=../admin',
      '/login?shop_product=https://evil.invalid',
      '/login?shop_product=a&shop_product=b',
      '/other?shop_product=hexciv-win64',
    ]) {
    ...
    expect(
      ShopLoginContinuation.loginUri(
        productId: '../admin',
        attribution: ShopAttribution.parse(),
      ).toString(),
      '/login',
    );
  });
  ...
  test('OAuth callback uses trusted origin, not input authority or credentials',
    ...
    final callback = ShopLoginContinuation.callbackUri(
      base:
          Uri.parse('https://store.example.invalid:8443/current?token=secret'),
  ...
  test('non-shop login keeps the existing default callback', () {
    expect(
      ShopLoginContinuation.callbackUri(
