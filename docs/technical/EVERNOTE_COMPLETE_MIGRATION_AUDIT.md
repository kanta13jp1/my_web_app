...
| タスク・リマインダー | 部分実装・UI/削除阻止をクラウド検証済み | 構造化Task、複数Taskリマインダー、ノート・リマインダーをowner-scopedネイティブ表へ原子的に保存。ノート内UIで作成・編集・完了切替・期限・RRULE・複数リマインダーを操作できる。件数と決定的SHA-256を読み戻し、未一致ならDBがEvernote側削除状態を拒否。移行原本は削除完了までDBとUIの双方で保護。en-todoはMarkdownチェックボックス化 | 担当者、通知、全体タスク絞り込み、実データ照合を完了 |
...
| タスク | ノート内ネイティブUI・保存・削除ゲートはクラウド検証済み | ENEX構造化Taskの期限、繰り返し、完了状態、複数リマインダーをノート内で操作でき、ハッシュを検証済み。担当、全体絞り込み、通知を完成 |
...
- クラウド優先CI: PR #5125
- ストリーミング移行・階層・履歴棚卸し/取込UI・ネイティブTask/リマインダー保存・ノート内Task管理UI・二重削除ゲート検証: PR #5127
- 実データ移行: 0件
- Evernote側削除: 0件
- サブスクリプション解約: 未実施
- 本番デプロイ: 未実施
