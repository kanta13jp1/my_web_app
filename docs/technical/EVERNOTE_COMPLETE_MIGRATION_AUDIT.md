...
| 保存済み検索 | 実装・クラウド検証済み／実データ未検証 | ENEX外のアカウント棚卸しJSONから検索名・検索条件原文・項目別SHA-256をowner-scoped表へ保存。端末間同期されるネイティブCRUD、検索画面への導線、明示的ゼロ件確認、検証RPCを備える。対応済みの文字列・タグ検索は実行でき、未対応のEvernote高度検索演算子も原文を失わず保持 | 件数、名称、検索条件原文を全件照合し、高度検索演算子の実行同等性を別途完了 |
...
| タグ・高度な検索 | 保存済み検索とショートカット管理はクラウド検証済み／高度演算子は部分実装 | 保存検索の原文保全・CRUD・ショートカット導線は実装済み。階層タグと全属性演算子の実行同等性を追加検証 |
...
7. タスク、リマインダー、保存済み検索、ショートカット、共有、内部リンクの移行確認。
