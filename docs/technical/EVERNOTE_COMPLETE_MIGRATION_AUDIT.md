...
| ノートブック | 未実装 | 現在はsource_contextメタデータのみ | 本サイトの正式ノートブックへ割当済み |
| スタック | 未実装 | ENEXはスタック構造を保持しないため手入力が必要 | スタック構造を別途棚卸しして正式構造へ復元 |
| Space | 未実装 | ENEXだけでは完全なSpace構造を保証できない | Space・権限・所属ノートを別途棚卸しして復元 |
| ノート履歴 | 未実装・削除阻止 | 通常のノートブックENEXだけでは履歴を網羅できない | 必要な履歴版を個別エクスポートし、復元確認 |
...
| ノートブック・スタック・Space | 未実装 | 階層の作成、移動、並び替え、検索、権限を実装 |
...
| ノート履歴・復元 | 本サイト側履歴あり／移行未実装 | Evernote履歴の保全と復元試験を完了 |
