...
- **実データ未検証**: 実アカウントの本番取込と原本・復旧コピーとの照合が証明されていない。ローカル保全や合成データのテスト成功だけでは完了としない。
...
| ノートブック | 実装・クラウド検証済み／実データ未検証 | owner-scoped階層へ正式割当し、レスポンシブ管理画面で作成、名称・説明編集、移動、ピン留め、表示順、既定指定、ノート表示、削除時の配下ノート退避を提供。既定ノートブックはDBでも削除拒否 | 全ノートの割当、名称、既定指定、並び順を実データ照合 |
| スタック | 実装・クラウド検証済み／実データ未検証 | ENEX外の構造を移行画面で入力して冪等復元し、ネイティブ作成、名称・説明編集、ピン留め、表示順、配下ノートブック管理を提供。公式仕様どおり新規スタックは個人用ルートとして作成 | 実アカウントのスタック棚卸し、個人別所属、全件動作を照合 |
| Space | 共有基盤・クラウド検証済み／実データ未検証 | Space名、説明、ピン留め、表示順、配下ノートブックに加え、メール招待、フルアクセス・編集・閲覧、受信一覧、直接所属ノート、同一Space内移動、退出を実装。Space内のノートとノートブックは権限を継承し、移行原本の名称・所属・削除は全項目のEvernote側削除確認までDBとUIで保護 | 実アカウント間の招待メール、直接所属ノート、権限、ピン留めを照合 |
...
| 共有・権限 | Space共有・クラウド検証済み／公開リンク未検証 | 所有者、フルアクセス、編集、閲覧の継承権限、メール本人だけの招待受諾、権限変更・取消・退出、部外者遮断をRLS/RPCとレスポンシブUIで実装。共有ノートはSpace所有者が保持し作成者を別記録 | 実アカウント間E2E、ノート単体共有、公開リンクを全件確認 |
...
| ノートブック・スタック・Space | ネイティブ管理・Space共有UIをクラウド検証済み | 作成、編集、ノートブック移動、ピン留め、並び替え、既定指定、削除確認、移行原本ロック、配下ノート表示、Space招待・権限・直接ノート・同一Space内移動・退出を実装。個人別スタック所属と実データ照合が残る |
| タグ・高度な検索 | 構文・属性検索・階層データモデル・管理UI・削除ゲートをクラウド検証済み／実データ未検証 | 大文字のAND/OR/NOTと括弧、any:、除外、引用句、末尾ワイルドカード、resource:/source:/todo:/contains:/encryption:を保存証拠から評価する。階層タグはowner-scoped CRUD/RLS/循環拒否/移行証拠に加え、狭幅/広幅管理UI、原本ロック、子タグ包含/除外フィルターを備える。人物等の証拠不足カテゴリは推測で肯定しない |
...
| 共有・Shared with Me | Space受信一覧・クラウド検証済み／公開リンク未実装 | メールに結び付いた保留招待の受諾・辞退、管理者による取消・権限変更、参加後のSpace一覧、退出後の即時失効を実装。実メール配信、ノート単体共有、公開リンクをE2E検証 |
...
- 高度検索のresource:/source:/todo:/contains:/encryption:、暗号化印保存、階層タグ管理UI、子タグ包含/除外のクラウド検証: commit `e0ceb2bbae8c44014126e3a8568a3c75cc06f383`、GitHub Actions run 33336463358（47 Dartファイル整形差分なし、Flutter analyze成功、対象114テスト成功、使い捨てPostgreSQLの階層・履歴・Task・通知・ナビゲーション・タグ/RLS/削除ゲート契約成功）
- Space共有・直接ノート・継承権限クラウド検証: commit `35b163bc0af88079a9a5ab5464fc9dac9737b37b`、GitHub Actions run 33340147866（54 Dartファイル整形差分なし、Flutter analyze成功、対象121テスト成功、使い捨てPostgreSQLの招待本人確認、owner/full_access/edit/view、部外者遮断、同一Space内移動、別Space拒否、退出後失効契約成功）
- 実データ移行: 0件
- Evernote側削除: 0件
- サブスクリプション解約: 未実施
- 本番デプロイ: 未実施
...
- Obsidianの第1バッチ管理記録は、ノート1件・添付8件（210,580 bytes）、欠損メディア0件を示す。全アカウントの総件数は未確定であり、これを全件棚卸しとは扱わない。
- 記録上の `obsidian_staged` は true、`site_production_imported`、`three_way_verified`、`evernote_deleted`、`subscription_cancelled` はすべて false。
- 元ENEXは現存し、ファイルサイズ360,686 bytesは記録と一致する。この回はRAM・ディスク保護のためENEX解析や再ハッシュをしていない。過去のハッシュ記録を新しい照合成功と扱わない。
- 元ENEXの保管庫へのコピーは記録上未実施。非公開クラウドへの原本保管と独立した復旧コピーの一致も未証明であり、削除ゲートは閉じたままとする。
- 現行バッチとバックアップはGitの除外対象であり、移行フォルダ内の現在のGit追跡ファイル数は0。本文・タイトル・パス・添付・認証情報はこの監査表に載せない。
- この回の本番取込、Evernote側削除、解約は未実施。

```text
元ENEXあり
  -> Obsidian第1バッチ保全（管理記録: 1ノート / 8添付）
  -> 非公開クラウド原本・復旧コピー照合 [未証明]
  -> 本サイト本番取込・三者照合 [未完了]
  -> 対象バッチの削除承認 [未取得]
  -> Evernote側削除
  -> 全アカウント / 全機能 / 復旧 / 請求監査
...
- 親PR #5125はドラフト。mainとの3競合を解消し、既存の業務CSVインポートとEvernoteストリーミングを両方保持した。
- commit `974f020043677ccf4b3eea22ebcf36088a017e75`: [クラウド検証](https://github.com/kanta13jp1/my_web_app/actions/runs/34669898441)で64ファイル整形差分なし、静的解析問題なし、対象159件・Chrome簡易2件・使い捨てDB契約成功。[Webリリースビルド](https://github.com/kanta13jp1/my_web_app/actions/runs/34669899501)も成功。Chrome簡易テストは認証済みENEX移行E2Eを証明しない。
- 同じコミットの[全体検証](https://github.com/kanta13jp1/my_web_app/actions/runs/34669890392)は3,390件成功・資産管理の優先順テスト1件失敗。この失敗は[未変更のmainでも再現](https://github.com/kanta13jp1/my_web_app/actions/runs/34669687869)した。
- 失敗したテストは実行日と既定請求項目に依存し、画面の上位8件という表示制限を並び順の検証と混同していた。月初・月途中・給料日・年末の固定日付を注入し、無関係な既定請求を支払済みにした合成fixtureで、両対象の表示とOFF→ON→OFFの順序を直接検証する。アプリ本体の計算・表示は変更しない。
- 修正したテストと関連ルールはcommit `a09fe013d24ae3d70f632bf8f68d3ee46a01f3e9`の[隔離クラウド検証](https://github.com/kanta13jp1/my_web_app/actions/runs/34670581573)で113件成功、整形差分0、静的解析問題なし。同じテストblobを親PRへ引き継ぐ。親PRの更新後全体CIは別途確認する。
- 必須のDesignプラグインによる画面監査、認証済み移行E2E、認証・RLS・権限・バッチ実行の人によるレビューは未完了。本番反映・移行完了の証拠として代用しない。
...
- 既定ノートブック: https://help.evernote.com/hc/en-us/articles/208314728-Change-your-default-notebook
- ノートブック削除: https://help.evernote.com/hc/en-us/articles/360051769514-Delete-a-notebook-Free-Personal-or-Professional
- Spaceとスタックの差異: https://help.evernote.com/hc/en-us/articles/208314678-FAQ-for-Evernote-Teams-users
