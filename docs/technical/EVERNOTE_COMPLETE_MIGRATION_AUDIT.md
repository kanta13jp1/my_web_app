...
| Space | 部分実装・クラウド検証済み／共有権限未実装 | ENEX外のSpace名をowner-scopedツリーへ冪等復元し、ネイティブ作成、説明、ピン留め、表示順、配下ノートブック管理を提供。移行原本の名称・所属・削除は全項目のEvernote側削除確認までDBとUIで保護 | Space・直接所属ノート・共有相手・権限・ピン留めを実データ照合 |
...
| ノートブック・スタック・Space | ネイティブ管理UI・クラウド検証済み／共有権限未実装 | 作成、編集、ノートブック移動、ピン留め、並び替え、既定指定、削除確認、移行原本ロック、配下ノート表示を実装。次にSpace共有権限、直接所属ノート、個人別スタック所属を実装 |
