...
| タグ・高度な検索 | 主要構文・階層データモデル・削除ゲートはクラウド検証済み／管理UIと一部属性演算子は未実装 | 大文字のAND/OR/NOTと括弧、any:、除外、引用句、末尾ワイルドカード、主要属性演算子を実行済み。階層タグのowner-scoped CRUD/RLS/循環拒否/移行証拠を実装済み。resource:/source:/todo:/contains:/encryption:の索引・実行と階層管理UIを追加検証 |
