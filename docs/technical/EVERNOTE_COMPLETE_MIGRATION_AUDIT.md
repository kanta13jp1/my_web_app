...
| タスク・リマインダー | 部分実装・削除阻止 | 構造化Task、複数Taskリマインダー、ノート・リマインダーをowner-scopedネイティブ表へ原子的に保存。件数と決定的SHA-256を読み戻し、未一致ならDBがEvernote側削除状態を拒否。en-todoはMarkdownチェックボックス化 | ネイティブTask UI、担当者、通知、実データ照合を完了 |
...
| タスク | ネイティブ保存・削除ゲートはクラウド検証済み／UI未実装 | ENEX構造化Taskを独立表へ保存し、期限、繰り返し、完了状態、複数リマインダーのハッシュを検証。ノート内編集、担当、絞り込み、通知をネイティブUIで完成 |
...
- クラウド優先CI: PR #5125
- ストリーミング移行・階層・履歴棚卸し/取込UI・ネイティブTask/リマインダー保存・二重削除ゲート検証: PR #5127
- 実データ移行: 0件
- Evernote側削除: 0件
- サブスクリプション解約: 未実施
- 本番デプロイ: 未実施
