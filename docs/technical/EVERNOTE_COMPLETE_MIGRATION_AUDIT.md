...
| タスク・リマインダー | 部分実装 | ENEX 4.0の構造化Task、期限、繰り返し、フラグ、作成者・編集者、複数リマインダーとノート・リマインダーを型付きsource_metadataへ保存。en-todoはMarkdownチェックボックス化 | ネイティブTask UIへの復元、担当者対応、実データ照合、削除ゲートを実装 |
...
| タスク | 部分実装 | ENEX構造化Taskの損失防止パーサーを実装。ノート内編集、期限、担当、完了状態、絞り込み、通知をネイティブUIで検証 |
...
- クラウド優先CI: PR #5125
- ストリーミング移行・階層・履歴棚卸し/取込UI・ENEX Task保全・削除ゲート検証: PR #5127
- 実データ移行: 0件
- Evernote側削除: 0件
- サブスクリプション解約: 未実施
- 本番デプロイ: 未実施
