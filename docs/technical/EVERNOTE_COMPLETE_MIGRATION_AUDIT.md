...
| ノート履歴 | 部分実装・削除阻止 | 個別ENEXをストリーム取込し、ENML・添付・SHA-256・日時を保持。履歴未確認ならDBがEvernote側削除を拒否 | 必要な履歴版を個別エクスポート・照合し、または履歴なしを明示確認 |
...
| ノート履歴・復元 | 個別履歴の移行基盤・削除ゲート実装／復元UI未検証 | 実履歴の保全、一覧・プレビュー・復元試験を完了 |
...
- クラウド優先CI: PR #5125
- ストリーミング移行・階層・履歴削除ゲート検証: PR #5127
- 実データ移行: 0件
- Evernote側削除: 0件
- サブスクリプション解約: 未実施
- 本番デプロイ: 未実施
