...
| resource: / source: / todo: / contains: / encryption: | 未実装 | 通常文字列へ誤変換せず、画面で未対応と表示して絞り込みを保留 |
| Booleanと属性演算子の保存済み検索 | 部分実装 | 対応済み構文は実行し、未対応構文も原文を損失なく保存 |
| 階層タグのクラウドデータモデル | 実装 | 親子、ノート割当、owner RLS、循環拒否、ネイティブCRUD |
| 階層タグの管理UI | 未実装 | 作成・移動・子タグ包含/除外UIが次工程 |
