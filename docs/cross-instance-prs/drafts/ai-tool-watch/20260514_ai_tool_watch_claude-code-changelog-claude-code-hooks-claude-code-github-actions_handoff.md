# AI Tool Watch Handoff 2026-05-14

Auto-generated by `scripts/ai_tool_watch_route.py`.

## Routing

- Owner lane: `Codex #1`
- WBS instance: `codex`
- Routing groups: `codex-runtime, hooks, schedule`
- Source report: `docs/ai-tool-watch/latest-report.md`
- Source JSON: `docs/ai-tool-watch/latest-report.json`
- PR skeleton: `docs/cross-instance-prs/drafts/ai-tool-watch/20260514_ai_tool_watch_claude-code-changelog-claude-code-hooks-claude-code-github-actions_pr_skeleton.md`

## Changed Sources

- **Claude Code changelog**
  - URL: https://code.claude.com/docs/en/changelog
  - Latest signal: `2.1.141 / May 13, 2026`
  - Keyword groups: `codex-runtime, hooks, integration, quality-cost, schedule`
- **Claude Code hooks reference**
  - URL: https://code.claude.com/docs/en/hooks
  - Latest signal: `Hooks reference - Claude Code Docs Skip to main content Claude Code Docs home page English`
  - Keyword groups: `codex-runtime, hooks, integration, quality-cost, schedule`
- **Claude Code GitHub Actions**
  - URL: https://code.claude.com/docs/en/github-actions
  - Latest signal: `Claude Code GitHub Actions - Claude Code Docs Skip to main content Claude Code Docs home pa`
  - Keyword groups: `codex-runtime, hooks, integration, quality-cost, schedule`
- **Codex changelog**
  - URL: https://developers.openai.com/codex/changelog
  - Latest signal: `2026-05-13 / Codex mobile documentation Added documentation for using Codex from the ChatGPT mobile app,`
  - Keyword groups: `codex-runtime, hooks, integration, quality-cost, schedule`
- **Codex use cases**
  - URL: https://developers.openai.com/codex/use-cases/
  - Latest signal: `Codex use cases Home API Docs Guides and concepts for the OpenAI API API reference Endpoint`
  - Keyword groups: `codex-runtime, hooks, integration, quality-cost, schedule`
- **Codex overview**
  - URL: https://openai.com/codex/
  - Latest signal: `AI Coding Partner from OpenAI`
  - Keyword groups: `codex-runtime, quality-cost, schedule`
- **Cursor changelog**
  - URL: https://cursor.com/changelog
  - Latest signal: `3.3 / May 7, 2026`
  - Keyword groups: `codex-runtime, integration, quality-cost, schedule`
- **Gemini Code Assist release notes**
  - URL: https://developers.google.com/gemini-code-assist/resources/release-notes
  - Latest signal: `VS Code Gemini Code Assist 2.82.0 / May 14, 2026`
  - Keyword groups: `hooks, integration, quality-cost, schedule`
- **Devin release notes**
  - URL: https://docs.devin.ai/release-notes
  - Latest signal: `May 13, 2026`
  - Keyword groups: `codex-runtime, hooks, integration, quality-cost, schedule`

## Delegation Packet

- WBS / Issue: created or updated by this router.
- Due date: next WBS sprint pickup unless an existing issue already owns the signal
- Current owners: Claude Code #1 + Codex #1 only.
- Objective: convert official tool changes into a concrete adoption decision and scoped implementation route.
- Allowed write set: docs, scripts, GitHub Actions, tests, and scoped product code after adoption decision.
- Prohibited write set: unrelated refactors, live secrets, dormant old Codex lanes.
- Required validation: unit tests for scripts, workflow syntax review, PR checks green, deploy/WBS Sync after merge.
- Memory/disk hygiene: record pre/post C: free space, top memory process, cleanup actions, and skipped parent-app processes.

## Result Contract

- Changed files:
- Validation result:
- PR / Issue links:
- Remaining risk:
- Next owner:
