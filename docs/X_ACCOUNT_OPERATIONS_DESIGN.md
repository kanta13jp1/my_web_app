...
## 4. 投稿カデンス (Cadence) — 2026-07-07 R8 改訂

- **目標 1-2 投稿/日**(ワンボタン投稿)。時間帯は JST 朝 (7-9 時) か夜 (20-22 時) を既定。
- **標準形式 = フルパッケージ**: ニュースフックの長文リード + スレッド(リプ 5-8) + ネイティブ投票(先頭リプ) + 字幕付き presenter 動画 + URL は最終リプ。旧「単発を基本」は supersede。
- カデンス/時間帯の実験は**計測ループ復旧後**(X 課金サイクル 2026-07-10 リセット + metrics 窓/二層 cron 着地後)に engagement データで最適化 (§6)。
- 画像/スクショは本番 UI の**実データ** ([REAL-DATA] / ダミー禁止)。
- **[SCHEDULE-WAKEUP] 整合**: 自動系 cron は深夜帯 (02-06 JST) を避ける。
- **自動投稿の棚卸し(2026-07-07)**: daily-report.yml 旧 Step 5 の viral-growth-engine / post-x-update は両関数とも削除済みの**死路だった**(恒常 404)→ report-only の投稿ナッジへ置換済み。固定テンプレの自動投稿は近似重複=スパム降格リスクのため復活させない。post-x-with-media.yml(週次)の同文テンプレも cap 逼迫中は停止 or 文面ローテを検討。
