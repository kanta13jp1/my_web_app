...
- Notion や Evernote、各種 SaaS と比較した「自分株式会社」独自機能の優位性を示すデモ画面を設計する
...
- 13の競合製品を機能的に凌駕するためのロードマップ進捗状況を、ホーム画面のプログレスバーと連動させて可視化・企画する
- 知的生産・資産管理・SNS・ビジネス・AI支援のすべてを網羅する、圧倒的な「自分株式会社」としての統合プラットフォーム企画を推進する
- ジョブカンに代表される「管理のための管理ツール」を廃し、「働く人を支援するAIアシスタント」としての自律型バックオフィス機能のコンセプトを策定する
 - ユーザーマニュアルを単なるヘルプではなく、競合からの移行時の学習コストを劇的に下げるための「教育コンテンツ」として企画する
- 個人からチーム・法人向けに拡張するためのアカウント階層と権限設計モデルを企画する

### 広告

- X、Meta、Google で少額テストを開始する
- import 訴求広告と AI 訴求広告の勝ち筋を比較する
- 競合13製品からの「乗り換え」を促す比較広告クリエイティブをA/Bテストし、Slack、Chatwork、MoneyForward利用者層に「情報・資産・フローの一元化」を訴求する
...
- 移行代行付き PoC を試す
- Notion、Evernote、Slack、Chatwork、ジョブカンなど複数 SaaS の連携疲れ・コスト高を起こしている企業に対し、一元化プラットフォーム「自分株式会社」をコスト削減策として提案する
- 業務効率化にとどまらず、従業員個人の資産形成・自己実現・知的生産までサポートできる唯一無二のウェルビーイングSaaSとしての価値を法人向けに訴求する
- 営業資料（Pitch Deck）に現在の開発実績と全13製品を上回る明確なロードマップを組み込む
...
- 開発実績（DevelopmentAchievementsCard）や、進捗バー（GrowthRoadmapProgressCard）を「ビルド・イン・パブリック」の証として対外的にアピールし、透明性でファンを獲得する
- ユーザー向け操作マニュアルをSEOコンテンツ（「〇〇から自分株式会社へ移行する方法」等）としても拡充させ、検索流入と利用開始時の離脱率防止（オンボーディング完了率向上）に直結させる
- Zenn 等の技術ブログから流入したエンジニアをアーリーアダプター（エバンジェリスト）として育成するマーケティングファネルを構築する

### 人事

- growth engineer と content marketer の採用要件を定義する
- Linterエラー0を徹底維持できる規律と、テストコードを書く文化を持つエンジニアの採用基準を策定する
- Supabase Edge Function、Deno、Flutter に精通し、かつデータモデリング能力に長けたフルスタック人材を確保する

### 経理

- CAC、LTV、回収期間を試算する
- 各種SaaS（Notion, Slack, ジョブカン, Chatwork, MoneyForwardなど）を自分株式会社アプリで一元化・代替することによる法人/個人の「自社コスト削減額（SaaSのライセンス費、API課金）」をアピール可能な実績としてトラッキング・経理処理する

### 調達

- AIモデル（Gemini, Claude）のAPI利用枠とコスト効率を常に比較し、最適なバックエンドを調達する
- 各プラットフォームへの技術ブログ・記事の水平展開を効率化するための外部リソース（編集者等）や自動化ツールを調達する
- Zenn, Qiita, はてな等への発信をサポートするコンテンツ制作の外注リソース、およびUI/UXデザイナーを調達する

### 事業計画

- Free、Pro、Team、Enterprise の収益モデルを定義する
- 登録者数目標（短期100、中期1万、長期1億）に対する進捗をリアルタイムダッシュボードで監視し、未達の場合は即座に施策をピボットする
- 現在2名の登録者から、13の競合製品すべてを上回るユーザー数（数億規模）に到達するという圧倒的なビジョンと、ダミーデータ無しの「本物の開発実績データと Edge Function による堅牢なアーキテクチャ」による透明性と実行力を、対外向け資金調達（シード・シリーズA）のコア資料に据える
