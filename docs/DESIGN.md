...
新規または大きく変更したUIでは、[Design Accessibility Audit](DESIGN_ACCESSIBILITY_AUDIT.md) のDesignプラグイン監査とPR証拠も必要です。監査が利用できない場合は未検証として報告し、Flutter実装の検証は `AGENTS.md` のcloud-first方針に従います。
