...
Do not use `git commit --no-verify` or `LEFTHOOK=0` for project work. The
`prepare-commit-msg` hook requires a fresh pre-commit success stamp for the
current Git tree, so a normal `--no-verify` commit is stopped locally. Git
sequencer operations such as rebase, merge, cherry-pick, and revert are allowed
to continue because they replay commits rather than create a new unchecked
working-tree commit.
...
- Claude Code owns hook design, quality-gate scope, and exception policy.
- Codex #1 owns cross-cutting implementation and UI/browser verification.
- Codex #2 owns CI, synchronization, GitHub Actions wiring, Edge Function
  checks, and deterministic automation drift.
